...
impl CredentialStore for JsonFileStore {
    ...
    async fn load(&self) -> Result<Option<KiroTokenInfo>, ProviderError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let content = std::fs::read_to_string(&path).map_err(|e| {
                ProviderError::Other(format!("Failed to read JSON creds {}: {e}", path.display()))
            })?;
            let data: serde_json::Value = serde_json::from_str(&content).map_err(|e| {
                ProviderError::Other(format!(
                    "Failed to parse JSON creds {}: {e}",
                    path.display()
                ))
            })?;

            let mut token =
                KiroTokenInfo::new(String::new(), CredentialSource::JsonFile(path.clone()));
    ...
    async fn save(&self, info: &KiroTokenInfo) -> Result<(), ProviderError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            data["expiresAt"] = serde_json::json!(
                DateTime::from_timestamp(info.expires_at, 0)
...
impl CredentialStore for SqliteStore {
    ...
    async fn load(&self) -> Result<Option<KiroTokenInfo>, ProviderError> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            .map_err(|e| {
                ProviderError::Other(format!("Failed to open SQLite {}: {e}", path.display()))
            })?;
                ...
                if let Ok(json_str) = res {
                    if let Ok(data) = serde_json::from_str::<serde_json::Value>(&json_str) {
                        if let Some(v) = data.get("client_id").and_then(|v| v.as_str()) {
                            info.client_id = Some(v.to_string());
                        }
                        if let Some(v) = data.get("client_secret").and_then(|v| v.as_str()) {
                            info.client_secret = Some(v.to_string());
                        }
                        ...
                        if let CredentialSource::SqliteDb {
                            ref mut reg_key, ..
                        } = info.source
                        {
...
impl CredentialStore for EnvStore {
    ...
    async fn load(&self) -> Result<Option<KiroTokenInfo>, ProviderError> {
        let rt = match std::env::var("GAUD_KIRO_REFRESH_TOKEN")
            .ok()
            .or_else(|| std::env::var("KIRO_REFRESH_TOKEN").ok())
        {
            Some(t) => t,
            None => return Ok(None),
        };
        ...
        token.region = std::env::var("GAUD_KIRO_REGION")
            .ok()
