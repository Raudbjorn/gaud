...
//!
//! Thin HTTP transport layer that delegates format conversion to ClaudeTransformer
//! and uses SseParser + ClaudeStreamState for streaming.
    ...
    ) -> Pin<Box<dyn std::future::Future<Output = Result<ChatResponse, ProviderError>> + Send + '_>>
    {
        ...
        Box::pin(async move {
            ...
            if !status.is_success() {
                ...
                if let Some(ctx_err) = detect_context_window_error(code, &text, "claude") {
                    return Err(ctx_err);
                }
            ...
            let (_, rate_limit_headers) = parse_rate_limit_headers(&resp_headers, "claude");
