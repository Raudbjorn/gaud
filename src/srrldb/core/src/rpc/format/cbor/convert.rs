            ...
            match t {
                ...
                TAG_GEOMETRY_LINE => match *v {
                    CborValue::Array(v) => {
                        ...
                        Ok(PublicValue::Geometry(PublicGeometry::Line(
                ...
                TAG_GEOMETRY_POLYGON => match *v {
                    CborValue::Array(v) if !v.is_empty() => {
                        ...
                        Ok(PublicValue::Geometry(PublicGeometry::Polygon(
                ...
                TAG_GEOMETRY_MULTIPOINT => match *v {
                    CborValue::Array(v) => {
                        ...
                        Ok(PublicValue::Geometry(PublicGeometry::MultiPoint(
                ...
                TAG_GEOMETRY_MULTILINE => match *v {
                    CborValue::Array(v) => {
                        ...
                        Ok(PublicValue::Geometry(PublicGeometry::MultiLine(
                ...
                TAG_GEOMETRY_MULTIPOLYGON => match *v {
                    CborValue::Array(v) => {
                        ...
                        Ok(PublicValue::Geometry(PublicGeometry::MultiPolygon(
                ...
                TAG_GEOMETRY_COLLECTION => match *v {
                    CborValue::Array(v) => {
                        ...
                        Ok(PublicValue::Geometry(PublicGeometry::Collection(
                            geometries,
                        )))
                    }
                    _ => Err(anyhow!("Expected a CBOR array with Geometry values")),
...
pub fn from_value(val: PublicValue) -> Result<CborValue> {
    match val {
        ...
        PublicValue::Number(v) => match v {
            ...
            PublicNumber::Decimal(v) => Ok(CborValue::Tag(
                TAG_STRING_DECIMAL,
        ...
        PublicValue::Uuid(v) => Ok(CborValue::Tag(
            TAG_SPEC_UUID,
        ...
        PublicValue::Table(v) => Ok(CborValue::Tag(
            TAG_TABLE,
            Box::new(CborValue::Text(v.into_string())),
        )),
        PublicValue::RecordId(PublicRecordId { table, key }) => Ok(CborValue::Tag(
            TAG_RECORDID,
            Box::new(CborValue::Array(vec![
...
fn from_record_id_key_range(r: PublicRecordIdKeyRange) -> Result<CborValue> {
    fn encode(b: Bound<PublicRecordIdKey>) -> Result<CborValue> {
        match b {
            Bound::Included(v) => Ok(CborValue::Tag(
                TAG_BOUND_INCLUDED,
                Box::new(from_record_id_key(v)?),
            )),
            Bound::Excluded(v) => Ok(CborValue::Tag(
                TAG_BOUND_EXCLUDED,
                Box::new(from_record_id_key(v)?),
            )),
            Bound::Unbounded => Ok(CborValue::Null),
...
fn to_record_id_key_range(val: CborValue) -> Result<PublicRecordIdKeyRange> {
    ...
    match val {
        CborValue::Array(v) if v.len() == 2 => {
            ...
            Ok(PublicRecordIdKeyRange { start, end })
        }
        _ => Err(anyhow!("Expected a CBOR array with 2 bounds")),
    }
}
...
fn from_record_id_key(v: PublicRecordIdKey) -> Result<CborValue> {
    match v {
        ...
        PublicRecordIdKey::Array(v) => from_array(v),
        PublicRecordIdKey::Object(v) => from_object(v),
        PublicRecordIdKey::Range(v) => Ok(CborValue::Tag(
            TAG_RANGE,
            Box::new(from_record_id_key_range(*v)?),
        )),
        PublicRecordIdKey::Uuid(v) => Ok(CborValue::Tag(
            TAG_SPEC_UUID,
            Box::new(CborValue::Bytes(v.into_bytes().into())),
        )),
    }
...
fn to_record_id_key(val: CborValue) -> Result<PublicRecordIdKey> {
    match val {
        ...
        CborValue::Tag(TAG_RANGE, v) => Ok(PublicRecordIdKey::Range(Box::new(
            to_record_id_key_range(*v)?,
        ))),
...
fn from_uuid(val: PublicUuid) -> CborValue {
    CborValue::Tag(
        TAG_SPEC_UUID,
        Box::new(CborValue::Bytes(val.into_bytes().into())),
    )
}
