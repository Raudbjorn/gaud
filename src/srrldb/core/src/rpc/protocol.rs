...
pub trait RpcProtocol {
    ...
    async fn attach(&self, session_id: Option<Uuid>) -> Result<DbResult, RpcError> {
        ...
        match session_id {
            Some(id) => {
                ...
                self.session_map()
                    .insert(Some(id), Arc::new(RwLock::new(session)));
    ...
    async fn execute(
        ...
        if !self.kvs().allows_rpc_method(&MethodTarget { method }) {
    ...
    async fn signup(
        ...
        let Some(PublicValue::Object(params)) = extract_args(params.into_vec()) else {
            return Err(RpcError::InvalidParams(
                "Expected (params:object)".to_string(),
            ));
        };
    ...
    async fn refresh(
        ...
        let out: Result<PublicValue> = token
            .refresh(self.kvs(), &mut session)
            .await
    ...
    async fn info(
        ...
        let mut res = self
            .kvs()
            .execute("SELECT * FROM $auth", &session, vars)
            .await?;
    ...
    async fn set(
        ...
        let Some((PublicValue::String(key), val)) =
            ...
            return Err(RpcError::InvalidParams(
                "Expected (key:string, value:Value)".to_string(),
            ));
        };
    ...
    async fn insert(
        ...
        // Process the method arguments
        let (what, data) = extract_args::<(PublicValue, PublicValue)>(params.into_vec()).ok_or(
            RpcError::InvalidParams("Expected (what:Value, data:Value)".to_string()),
        )?;
    ...
    async fn create(
        ...
        let (what, data) = extract_args::<(PublicValue, Option<PublicValue>)>(params.into_vec())
            .ok_or(RpcError::InvalidParams(
                "Expected (what:Value, data:Value)".to_string(),
            ))?;
    ...
    async fn patch(
        ...
        let mut res = self
            .kvs()
            .process(Ast::single_expr(expr), &session, var)
            .await?;
        // Extract the first query result
        let first = res.remove(0).result?;
        Ok(DbResult::Other(first))
    ...
    async fn query(
        ...
        let PublicValue::String(query) = query else {
            return Err(RpcError::InvalidParams(
                "Expected query to be string".to_string(),
            ));
        };
        ...
        Ok(DbResult::Query(
            run_query(self, txn, session_id, QueryForm::Text(&query), vars).await?,
        ))
    }
    ...
    async fn run(
        ...
        let args = match args {
            Some(PublicValue::Array(args)) => args
                .into_iter()
                .map(Expr::from_public_value)
                .collect::<Vec<Expr>>(),
        ...
        let name = match segments.first() {
            ...
            Some(&"silo") => {
                ...
                let Some(pkg) = segments.get(2).map(|x| (*x).to_string()) else {
                    return Err(RpcError::InvalidParams(
                        "Expected silo package name".to_string(),
                    ));
                };
    ...
    let res = if let Some(txn_id) = txn {
        ...
        match query {
            QueryForm::Text(query) => {
                this.kvs()
                    .execute_with_transaction(query, &session, vars, tx)
                    .await?
            }
            QueryForm::Parsed(ast) => {
                this.kvs()
                    .process_with_transaction(ast, &session, vars, tx)
                    .await?
            }
        }
    } else {
