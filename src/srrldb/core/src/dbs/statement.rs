...
impl Statement<'_> {
    ...
    pub(crate) fn expr(&self) -> Option<&Fields> {
        match self {
            Statement::Select { stmt, .. } => Some(&stmt.fields),
    ...
    pub(crate) fn cond(&self) -> Option<&Cond> {
        match self {
            ...
            Statement::Select { stmt, .. } => stmt.cond.as_ref(),
    ...
    pub(crate) fn split(&self) -> Option<&Splits> {
        match self {
            Statement::Select { stmt, .. } => stmt.split.as_ref(),
            _ => None,
        }
    }
    ...
    pub(crate) fn group(&self) -> Option<&Groups> {
        match self {
            Statement::Select { stmt, .. } => stmt.group.as_ref(),
            _ => None,
        }
    }
    ...
    pub(crate) fn order(&self) -> Option<&Ordering> {
        match self {
            Statement::Select { stmt, .. } => stmt.order.as_ref(),
            _ => None,
        }
    }
    ...
    pub(crate) fn with(&self) -> Option<&With> {
        match self {
            Statement::Select { stmt, .. } => stmt.with.as_ref(),
    ...
    pub(crate) fn fetch(&self) -> Option<&Fetchs> {
        match self {
            Statement::Select { stmt, .. } => stmt.fetch.as_ref(),
            _ => None,
        }
    }
    ...
    pub(crate) fn start(&self) -> Option<&Start> {
        match self {
            Statement::Select { stmt, .. } => stmt.start.as_ref(),
            _ => None,
        }
    }
    ...
    pub(crate) fn limit(&self) -> Option<&Limit> {
        match self {
            Statement::Select { stmt, .. } => stmt.limit.as_ref(),
            _ => None,
        }
    }
    ...
    pub(crate) fn omit(&self) -> &[Idiom] {
        match self {
            Statement::Select { omit, .. } => omit.as_slice(),
            _ => &[],
        }
    }
    ...
    pub(crate) fn is_only(&self) -> bool {
        match self {
            ...
            Statement::Select { stmt, .. } => stmt.only,
    ...
    pub(crate) fn tempfiles(&self) -> bool {
        match self {
            Statement::Select { stmt, .. } => stmt.tempfiles,
            _ => false,
        }
    }
    ...
    pub(crate) fn explain(&self) -> Option<&Explain> {
        match self {
            Statement::Select { stmt, .. } => stmt.explain.as_ref(),
    ...
    pub(crate) fn timeout(&self) -> Option<&Expr> {
        match self {
            ...
            Statement::Select { stmt, .. } => Some(&stmt.timeout),
