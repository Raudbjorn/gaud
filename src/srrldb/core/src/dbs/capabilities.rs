...
impl fmt::Display for ParseNetTargetError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "The provided network target is not a valid host name, IP address or CIDR block"
        )
    }
}
...
mod tests {
    ...
    async fn test_net_target_resolve_async() {
        ...
        // runners.
        let r = NetTarget::from_str("localhost")
            .unwrap()
            .resolve()
            .await
            .unwrap();
