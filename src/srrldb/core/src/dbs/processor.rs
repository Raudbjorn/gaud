...
impl Collector for ConcurrentCollector<'_> {
    ...
    async fn collect(&mut self, collectable: Collectable) -> Result<()> {
        ...
        self.ite
            .process(self.stk, self.ctx, self.opt, self.stm, pro)
            .await?;
...
impl Collector for ConcurrentDistinctCollector<'_> {
    ...
    async fn collect(&mut self, collectable: Collectable) -> Result<()> {
        ...
        let pro = collectable
            .prepare(self.coll.opt, self.coll.txn, skippable)
            .await?;
...
pub(super) trait Collector {
    ...
    async fn collect_iterable(
        ...
        match iterable {
            ...
            Iterable::GenerateRecordId(doc_ctx, v) => {
                self.collect(Collectable::GenerateRecordId(doc_ctx, v))
                    .await?
            }
            ...
            Iterable::Lookup {
                ...
                self.collect_lookup(ctx, opt, doc_ctx, from, kind, what)
                    .await?
            }
            ...
            Iterable::Range(doc_ctx, tb, v, rs, sc) => match rs {
                ...
                RecordStrategy::KeysOnly => {
                    self.collect_range_keys(ctx, opt, doc_ctx, &tb, v, sc)
                        .await?
                }
            ...
            Iterable::Table(doc_ctx, table, rs, sc) => {
                ...
                match rs {
                    ...
                    RecordStrategy::KeysOnly => {
                        self.collect_table_keys(&ctx, opt, doc_ctx, &table, sc)
                            .await?
                    }
                    RecordStrategy::KeysAndValues => {
            ...
            Iterable::Mergeable(doc_ctx, tb, id, o) => {
                self.collect(Collectable::Mergeable(doc_ctx, tb, id, o))
                    .await?
            }
    ...
    async fn collect_table(
        ...
        while let Some(res) = stream.next().await {
            ...
            // Parse the data from the store
            let (k, v) = res?;
            self.collect(Collectable::KeyVal(doc_ctx.clone(), k, v))
                .await?;
            count += 1;
        }
        // Everything ok
    ...
    async fn collect_table_keys(
        ...
        while let Some(res) = stream.next().await {
            ...
            let k = res?;
            // Collect the key
            self.collect(Collectable::TableKey(doc_ctx.clone(), k))
                .await?;
            count += 1;
        }
        // Everything ok
    ...
    async fn collect_range_keys(
        ...
        while let Some(res) = stream.next().await {
            ...
            // Parse the data from the store
            let k = res?;
            self.collect(Collectable::RangeKey(doc_ctx.clone(), k))
                .await?;
            count += 1;
        }
        // Everything ok
    ...
    async fn collect_lookup(
        ...
        let keys = match (what.is_empty(), &kind) {
            (true, LookupKind::Reference) => {
                vec![(
                    r#ref::prefix(ns, db, tb, id)?,
                    r#ref::suffix(ns, db, tb, id)?,
                )]
            }
            (true, LookupKind::Graph(dir)) => match dir {
                ...
                Dir::Both => {
                    vec![(
                        graph::prefix(ns, db, tb, id)?,
                        graph::suffix(ns, db, tb, id)?,
                    )]
                }
                // /ns/db/tb/id/IN
                Dir::In => vec![(
            ...
            (false, kind) => what
                .iter()
                .map(|v| v.presuf(ns, db, tb, id, kind))
                .collect::<Result<Vec<_>>>()?,
        };
        // Get the transaction
        let txn = ctx.tx();
        ...
        for (beg, end) in keys {
            ...
            while let Some(res) = stream.next().await {
                ...
                let key = res?;
                // Collector the key
                self.collect(Collectable::Lookup(doc_ctx.clone(), kind.clone(), key))
                    .await?;
                count += 1;
            }
        }
    ...
    async fn collect_index_items(
        ...
        let Some(iterator) = exe
            .new_iterator(doc_ctx.ns.namespace_id, doc_ctx.db.database_id, irf)
            .await?
        else {
            bail!(Error::QueryNotExecuted {
        ...
        match rs {
            RecordStrategy::Count => {
                self.collect_index_item_count(ctx, &txn, doc_ctx, iterator)
                    .await?
            }
            RecordStrategy::KeysOnly => {
                self.collect_index_item_key(ctx, &txn, doc_ctx, iterator)
                    .await?
            }
            RecordStrategy::KeysAndValues => {
                self.collect_index_item_key_value(ctx, &txn, doc_ctx, iterator)
                    .await?
            }
        }
        // Everything ok
    ...
    async fn collect_index_item_key(
        ...
        while !ctx.is_done(None).await? {
            ...
            for (count, record) in records.into_iter().enumerate() {
                ...
                self.collect(Collectable::IndexItemKey(doc_ctx.clone(), record))
                    .await?;
            }
        }
        Ok(())
    ...
    async fn collect_index_item_key_value(
        ...
        while !ctx.is_done(None).await? {
            ...
            for (count, record) in records.into_iter().enumerate() {
                ...
                self.collect(Collectable::IndexItem(doc_ctx.clone(), record))
                    .await?;
            }
        }
        Ok(())
