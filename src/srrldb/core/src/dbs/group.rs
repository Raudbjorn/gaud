...
impl GroupCollector {
    ...
    pub(super) fn explain(&self, exp: &mut Explanation) {
        ...
        let selector = match &self.analysis.fields {
            ...
            AggregateFields::Fields(items) => items
                .iter()
                .map(|(k, v)| (k.to_sql(), Value::from(v.to_sql())))
                .collect(),
        };
    ...
    pub async fn push(
        ...
        for g in self.analysis.group_expressions.iter() {
            let v = stk
                .run(|stk| g.compute(stk, ctx, opt, Some(&doc)))
                .await
                .catch_return()?;
        ...
        if let RecordStrategy::Count = rs {
            ...
            for a in aggragates.iter_mut() {
                if let AggregationStat::Count { count } = a {
                    *count = n.as_int();
                }
            }
        } else {
            ...
            for v in self.analysis.aggregate_arguments.iter() {
                let v = stk
                    .run(|stk| v.compute(stk, ctx, opt, Some(&doc)))
                    .await
                    .catch_return()?;
        ...
        for (group, result) in std::mem::take(&mut self.results) {
            ...
            for (idx, a) in result.into_iter().enumerate() {
                ...
                let value = match a {
                    ...
                    | AggregationStat::CountValue { count, .. } => Value::from(Number::from(count)),
                    ...
                    AggregationStat::Mean { sum, count, .. } => sum
                        ...
                        .into(),
                    AggregationStat::TimeMax { max, .. } => max.into(),
                    AggregationStat::TimeMin { min, .. } => min.into(),
                    AggregationStat::Accumulate { values, .. } => values.into(),
