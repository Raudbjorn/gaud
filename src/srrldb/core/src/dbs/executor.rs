...
impl Executor {
    ...
    async fn execute_plan_impl(
        ...
        match self
            .execute_plan_in_transaction(txn.clone(), start, plan)
            .await
        {
        ...
        S: Stream<Item = Result<TopLevelExpr>>,
    {
        ...
        while let Some(stmt) = stream.next().await {
            ...
            let result = match stmt {
                ...
                TopLevelExpr::Commit => {
                    ...
                    for res in &mut self.results[start_results..] {
                        res.query_type = QueryType::Other;
                        res.result = Err(DbResultError::InternalError(format!(
                            "Query not executed: {}",
                            e
                        )));
                    }
                ...
                stmt => {
                    ...
                    let r = match self
                        .execute_plan_in_transaction(txn.clone(), &before, plan)
                        .await
                    {
                        Ok(x) => Ok(x),
        ...
        for res in &mut self.results[start_results..] {
            res.query_type = QueryType::Other;
            res.result = Err(DbResultError::InternalError(
    ...
    pub(crate) async fn execute_plan_with_transaction(
        ...
        for expr in qry.expressions {
            ...
            let result = executor
                .execute_plan_in_transaction(tx.clone(), &start, expr)
                .await;
...
mod tests {
    ...
    async fn check_execute_timeout() {
        // With small timeout
        {
            ...
            let res = ds
                .execute(stmt, &Session::default().with_ns("NS").with_db("DB"), None)
                .await;
            assert!(
                res.is_ok(),
                "Failed to execute statement with small timeout: {:?}",
                res
            );
        }
        // With large timeout
        {
            ...
            assert!(
                res.is_ok(),
                "Failed to execute statement with large timeout: {:?}",
                res
            );
        }
        // With very large timeout
        {
            ...
            assert!(
                res.is_ok(),
                "Failed to execute statement with very large timeout: {:?}",
                res
            );
