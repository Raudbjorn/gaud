...
impl Explanation {
    ...
    pub(super) fn add_collector(
        ...
        self.0
            .push(ExplainItem::new_collector(collector_type, details));
    }
    ...
    pub(super) fn add_start_limit(
        ...
        self.0
            .push(ExplainItem::new_start_limit(start_skip, cancel_on_limit));
    }
...
impl ExplainItem {
    ...
    fn new_iter(ctx: &FrozenContext, iter: &Iterable) -> Self {
        match iter {
            ...
            Iterable::Lookup { from, kind, .. } => match kind {
            ...
            Iterable::Range(_doc_ctx, tb, r, rs, sc) => Self {
                ...
                details: vec![
                    ...
                    (
                        "range",
                        Value::Range(Box::new(r.clone().into_value_range())),
                    ),
                    ("direction", sc.to_string().into()),
                ],
            },
            ...
            Iterable::Mergeable(_doc_ctx, tb, Some(id), v) => Self {
                ...
                details: vec![
                    (
                        "record",
                        Value::RecordId(RecordId::new(tb.to_owned(), id.to_owned())),
                    ),
                    ("value", v.to_owned()),
                ],
            },
...
impl From<ExplainItem> for Value {
    fn from(i: ExplainItem) -> Self {
        let explain = Object::from(HashMap::from([
            ...
            (
                "detail",
                Value::Object(Object::from(HashMap::from_iter(i.details))),
            ),
        ]));
        Value::from(explain)
    }
