...
impl Iterator {
    ...
    pub(crate) async fn prepare(
        ...
        match val {
            ...
            Expr::Literal(Literal::Array(array)) => {
                self.prepare_array(stk, ctx, opt, doc, planner, stm_ctx, doc_ctx, array)
                    .await?
            }
            x => {
                self.prepare_computed(stk, ctx, opt, doc, planner, stm_ctx, doc_ctx, x)
                    .await?
            }
        };
        // All ingested ok
    ...
    pub(crate) async fn prepare_table(
        ...
        let tb = if stm_ctx.stm.requires_table_existence() {
            ctx.tx()
                .expect_tb(doc_ctx.ns.namespace_id, doc_ctx.db.database_id, table)
                .await?
        } else {
            ctx.tx()
                .get_or_add_tb(Some(ctx), &doc_ctx.ns.name, &doc_ctx.db.name, table)
                .await?
        };
        ...
        planner
            .add_iterables(stk, stm_ctx, doc_ctx, table, granted_perms, self)
            .await?;
    ...
    pub(crate) async fn prepare_record_id(
        ...
        let tb = if stm_ctx.stm.requires_table_existence() {
            ctx.tx()
                .expect_tb(doc_ctx.ns.namespace_id, doc_ctx.db.database_id, &rid.table)
                .await?
        } else {
            ctx.tx()
    ...
    pub(crate) async fn prepare_mock(
        ...
        mock: &Mock,
            ...
            !stm_ctx.stm.is_only() || self.is_limit_one_or_zero(),
    ...
    pub(crate) async fn prepare_lookup(
        ...
        what: Vec<ComputedLookupSubject>,
            ...
            !stm.is_only() || self.is_limit_one_or_zero(),
            Error::SingleOnlyOutput
        );
        ...
        if stm.is_create() {
            ...
            bail!(Error::InvalidStatementTarget { value })
        }

        let txn = ctx.tx();
        let tb = if stm.requires_table_existence() {
            txn.expect_tb(doc_ctx.ns.namespace_id, doc_ctx.db.database_id, &from.table)
                .await?
        } else {
            txn.get_or_add_tb(Some(ctx), &doc_ctx.ns.name, &doc_ctx.db.name, &from.table)
                .await?
        };
        let fields = txn
            .all_tb_fields(
    ...
    async fn prepare_computed(
        ...
        match v {
            ...
            Value::RecordId(rid) => {
                self.prepare_record_id(ctx, opt, planner, stm_ctx, doc_ctx, rid)
                    .await?
            }
            Value::Array(array) => {
                for v in array {
                    match v {
                        ...
                        v => {
                            bail!(Error::InvalidStatementTarget { value: v.to_sql() })
                        }
                    }
                }
            }
            ...
            Value::Object(o) => {
                if let Some(id) = o.rid() {
                    ...
                    bail!(Error::InvalidStatementTarget { value: o.to_sql() })
                }
            }
            v => {
                bail!(Error::InvalidStatementTarget { value: v.to_sql() })
        ...
        if plan.do_iterate {
            ...
            if self.results.is_empty() {
                ...
                if let Some(guaranteed) = self.guaranteed.take() {
                    ...
                    self.iterate(stk, ctx, opt, stm, is_specific_permission, None)
                        .await?;
                }
            }
            ...
            self.results
                .start_limit(self.start_skip, self.start, self.limit)
                .await?;
    ...
    async fn iterate(
        ...
        for (count, v) in mem::take(&mut self.entries).into_iter().enumerate() {
            v.iterate(stk, ctx, &opt, stm, self, distinct.as_mut())
                .await?;
    ...
    async fn extract_value(
        ...
        stk.run(|stk| Document::process(stk, ctx, opt, stm, pro))
            .await
    }
