...
impl<'a> IndexOperation<'a> {
    ...
    pub(crate) async fn index_count_compaction(
        ...
        IndexCountThingIterator::new(ic.ns, ic.db, ic.tb.as_ref(), ic.ix)?
            .compaction(ic, tx)
            .await
    }
    ...
    async fn index_fulltext(
        ...
        let doc_id = if let Some(o) = self.o.take() {
            fti.remove_content(stk, self.ctx, self.opt, self.rid, o, &mut rc)
                .await?
        } else {
            None
        };
        // Create the new index data
        if let Some(n) = self.n.take() {
            fti.index_content(stk, self.ctx, self.opt, self.rid, n, &mut rc)
                .await?;
        } else {
