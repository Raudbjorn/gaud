...
impl Analyzer {
    ...
    pub(in crate::idx::ft) async fn analyze_content(
        ...
        for v in content {
            self.analyze_value(stk, ctx, opt, v, stage, &mut tks)
                .await?;
        }
        Ok(tks)
    }
    ...
    pub(super) async fn analyze_value(
        ...
        match val {
            ...
            Value::Number(n) => tks.push(
                self.generate_tokens(stk, ctx, opt, stage, n.to_sql())
                    .await?,
            ),
            Value::Bool(b) => tks.push(
                self.generate_tokens(stk, ctx, opt, stage, b.to_sql())
                    .await?,
            ),
            Value::Array(a) => {
                for v in a.0 {
                    stk.run(|stk| self.analyze_value(stk, ctx, opt, v, stage, tks))
                        .await?;
                }
            }
            Value::Object(o) => {
    ...
    pub(crate) async fn analyze(
        ...
        self.generate_tokens(stk, ctx, opt, FilteringStage::Indexing, input)
            .await?
            .try_into()
    }
...
mod tests {
    ...
    async fn get_analyzer_tokens(def: &str, input: &str) -> Tokens {
        ...
        let Expr::Define(d) = expr else { panic!() };
