...
impl FullTextIndex {
    ...
    fn with_analyzer(
        ...
        let mut bm25 = None;
        if let Scoring::Bm { k1, b } = p.scoring {
            bm25 = Some(Bm25Params { k1, b });
        }
        Ok(Self {
            analyzer,
    ...
    pub(crate) async fn remove_content(
        ...
        let tokens = self
            .analyzer
            .analyze_content(stk, ctx, opt, content, FilteringStage::Indexing)
            .await?;
    ...
    pub(crate) async fn index_content(
        ...
        let dl = if self.highlighting {
            self.index_with_offsets(&nid, &tx, id.doc_id(), tokens)
                .await?
        } else {
            self.index_without_offsets(&nid, &tx, id.doc_id(), tokens)
                .await?
        };
        {
            // Set the doc length
        ...
        }
        {
            ...
            let key = self
                .ikb
                .new_dc_with_id(id.doc_id(), opt.id(), Uuid::now_v7());
            let dcl = DocLengthAndCount {
                total_docs_length: dl as i128,
                doc_count: 1,
            };
    ...
    pub(in crate::idx) async fn matches_value(
        ...
        let mut tks = vec![];
        self.analyzer
            .analyze_value(stk, ctx, opt, val, FilteringStage::Indexing, &mut tks)
            .await?;
        match bo {
    ...
    async fn compact_doc_length_and_count(&self, tx: &Transaction) -> Result<bool> {
        ...
        let dlc = self
            .compute_doc_length_and_count(tx, Some(&mut has_logs))
            .await?;
    ...
    pub(in crate::idx) async fn highlight(
        ...
        if let Some(doc_id) = doc_id {
            ...
            for tk in qt.tokens.list() {
                if let Some(td) = self
                    .get_term_document(tx, doc_id, qt.tokens.get_token_string(tk)?)
                    .await?
                {
...
mod tests {
    ...
    impl TestContext {
        async fn new() -> Self {
            ...
            let Expr::Define(q) = q else { panic!() };
        ...
        async fn remove_insert_task(&self, stk: &mut Stk, rid: &RecordId) {
            ...
            if require_compaction {
                FullTextIndex::trigger_compaction(&self.ikb, &tx, self.nid)
                    .await
                    .unwrap();
            }
    ...
    async fn concurrent_doc_update(test: TestContext, rid: Arc<RecordId>, mut count: usize) {
        ...
        while count > 0 && test.start.elapsed().as_millis() < 3000 {
            stack
                .enter(|stk| test.remove_insert_task(stk, &rid))
                .finish()
                .await;
            count -= 1;
        }
    ...
    async fn concurrent_test() {
        ...
        let task1 = tokio::spawn(concurrent_doc_update(
            test.clone(),
            doc1.clone(),
            usize::MAX,
        ));
        let task2 = tokio::spawn(concurrent_doc_update(
            test.clone(),
            doc2.clone(),
            usize::MAX,
        ));
