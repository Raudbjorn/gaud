...
impl HnswFlavor {
    pub(super) fn new(
        ...
        let res = match p.m {
            1..=4 => match p.m0 {
                ...
                _ => Self::H5set(Hnsw::<AHashSet, ArraySet<5>>::new(
                    table_id,
                    ibk,
                    p,
                    vector_cache,
            ...
            5..=8 => match p.m0 {
                ...
                _ => Self::H9set(Hnsw::<AHashSet, ArraySet<9>>::new(
                    table_id,
                    ibk,
                    p,
                    vector_cache,
            ...
            13..=16 => Self::H17set(Hnsw::<AHashSet, ArraySet<17>>::new(
                table_id,
                ibk,
                p,
                vector_cache,
            )?),
            17..=20 => Self::H21set(Hnsw::<AHashSet, ArraySet<21>>::new(
                table_id,
                ibk,
                p,
                vector_cache,
            )?),
            21..=24 => Self::H25set(Hnsw::<AHashSet, ArraySet<25>>::new(
                table_id,
                ibk,
                p,
                vector_cache,
            )?),
            25..=28 => Self::H29set(Hnsw::<AHashSet, ArraySet<29>>::new(
                table_id,
                ibk,
                p,
                vector_cache,
            )?),
            _ => Self::Hset(Hnsw::<AHashSet, AHashSet>::new(
                table_id,
                ibk,
                p,
                vector_cache,
    ...
    pub(super) async fn knn_search_checked(
        ...
        match self {
            HnswFlavor::H5_9(h) => {
                h.knn_search_checked(db, tx, stk, search, hnsw_docs, vec_docs, chk)
                    .await
            }
            HnswFlavor::H5_17(h) => {
                h.knn_search_checked(db, tx, stk, search, hnsw_docs, vec_docs, chk)
