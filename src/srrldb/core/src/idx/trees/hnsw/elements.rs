...
impl HnswElements {
    ...
    pub(super) async fn contains(&self, e_id: ElementId) -> bool {
        self.vector_cache
            .contains(self.table_id, self.index_id, e_id)
            .await
    }
    ...
    pub(super) async fn insert(
        ...
        self.vector_cache
            .insert(self.table_id, self.index_id, id, pt.clone())
            .await;
        Ok(pt)
    }
    ...
    pub(super) async fn get_vector(
        ...
        if let Some(v) = self
            .vector_cache
            .get(self.table_id, self.index_id, *e_id)
            .await
        {
            return Ok(Some(v));
        }
        ...
        match tx.get(&key, None).await? {
            ...
            Some(vec) => {
                ...
                self.vector_cache
                    .insert(self.table_id, self.index_id, *e_id, vec.clone())
                    .await;
                Ok(Some(vec))
            }
        }
