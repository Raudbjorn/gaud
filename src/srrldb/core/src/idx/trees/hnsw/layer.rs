    ...
    S: DynamicSet,
{
    ...
    pub(super) async fn search_single(
        ...
        self.search(tx, elements, pt, candidates, visited, w, ef)
            .await
    }
    ...
    pub(super) async fn search_single_with_ignore(
        ...
        let q = self
            .search(tx, elements, pt, candidates, visited, w, ef)
            .await?;
    ...
    pub(super) async fn search_single_checked(
        ...
        self.search_checked(db, tx, stk, search, candidates, visited, w, chk)
            .await
    }
    ...
    pub(super) async fn search_multi_with_ignore(
        ...
        let w = DoublePriorityQueue::default();
        self.search(tx, elements, pt, candidates, visited, w, efc)
            .await
    }
    ...
    pub(super) async fn insert(
        ...
        let mut neighbors = self.graph.new_edges();
        {
            ...
            eps = w.clone();
            heuristic
                .select(tx, elements, self, q_id, q_pt, w, None, &mut neighbors)
                .await?;
        };
