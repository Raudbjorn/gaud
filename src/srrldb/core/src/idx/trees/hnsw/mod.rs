...
impl KVValue for HnswState {
    ...
    fn kv_decode_value(val: Vec<u8>) -> Result<Self> {
        Ok(DeserializeRevisioned::deserialize_revisioned(
            &mut val.as_slice(),
        )?)
    }
}
    ...
    L: DynamicSet,
{
    ...
    async fn insert_level(
        ...
        for i in top_up_layers..q_level {
            self.layers
                .push(HnswLayer::new(self.ikb.clone(), i + 1, self.m));
        ...
        if let Some(ep_id) = self.state.enter_point {
            ...
            self.insert_element(tx, q_id, &q_pt, q_level, ep_id, top_up_layers)
                .await?;
        } else {
    ...
    async fn insert_first_element(
        ...
        // Insert in layer 0
        self.layer0
            .add_empty_node(tx, id, &mut self.state.layer0)
            .await?;
    ...
    async fn remove(&mut self, tx: &Transaction, e_id: ElementId) -> Result<bool> {
        ...
        if let Some(e_pt) = self.elements.get_vector(tx, &e_id).await? {
                ...
                .rev()
            {
                ...
                if layer
                    .remove(tx, st, &self.elements, &self.heuristic, e_id, self.efc)
                    .await?
                {
                    removed = true;
                }
...
mod tests {
    ...
    async fn test_hnsw_collection(p: &HnswParams, collection: &TestCollection) {
        ...
        let map = {
            let tx = ds
                .transaction(TransactionType::Write, Optimistic)
                .await
                .unwrap();
    ...
    async fn insert_collection_hnsw_index(
        ...
        for (doc_id, obj) in collection.to_vec_ref() {
            ...
            h.index_document(tx, &RecordIdKey::Number(*doc_id as i64), &content)
                .await
                .unwrap();
    ...
    async fn delete_hnsw_index_collection(
        ...
        for (doc_id, obj) in collection.to_vec_ref() {
            let content = vec![Value::from(obj.deref())];
            h.remove_document(tx, RecordIdKey::Number(*doc_id as i64), &content)
                .await?;
    ...
    async fn test_hnsw_index(collection_size: usize, unique: bool, p: HnswParams) {
        ...
        let (mut h, map) = {
            ...
            .unwrap();
            // Fill index
            let map = insert_collection_hnsw_index(&tx, &mut h, &collection)
                .await
                .unwrap();
            tx.commit().await.unwrap();
            (h, map)
        ...
        // Delete collection
        {
            let ctx = new_ctx(&ds, TransactionType::Write).await;
            let tx = ctx.tx();
            delete_hnsw_index_collection(&tx, &mut h, &collection, map)
                .await
                .unwrap();
            tx.commit().await.unwrap();
        }
