...
impl Vector {
        ...
        T: ToFloat,
    {
        Zip::from(a)
            .and(b)
            .map_collect(|x, y| (x.to_float() - y.to_float()).powi(2))
            .sum()
            .sqrt()
    }
        ...
        T: PartialEq,
    {
        Zip::from(a)
            .and(b)
            .fold(0, |acc, a, b| if a != b { acc + 1 } else { acc }) as f64
    }
        ...
        T: Eq + Hash + Clone,
    {
        ...
        let intersection_size =
            b.iter()
                .cloned()
                .fold(0, |acc, n| if !union.insert(n) { acc + 1 } else { acc }) as f64;
        intersection_size / union.len() as f64
    }
...
impl From<&Vector> for Value {
    fn from(v: &Vector) -> Self {
        let vec: Vec<_> = match v {
            Vector::F64(a) => a
                .iter()
                .map(|i| Number::Float(*i))
                .map(Value::from)
                .collect(),
            Vector::F32(a) => a
                .iter()
                .map(|i| Number::Float(*i as f64))
                .map(Value::from)
                .collect(),
            Vector::I64(a) => a.iter().map(|i| Number::Int(*i)).map(Value::from).collect(),
            Vector::I32(a) => a
                .iter()
                .map(|i| Number::Int(*i as i64))
                .map(Value::from)
                .collect(),
            Vector::I16(a) => a
                .iter()
...
impl Vector {
        ...
        T: TryFrom<Number, Error = Error>,
    {
        match value {
            ...
            _ => Err(anyhow::Error::new(Error::InvalidVectorValue(
                value.clone().to_raw_string(),
            ))),
        }
    }
    ...
    pub(super) fn check_expected_dimension(current: usize, expected: usize) -> Result<()> {
            ...
            current == expected,
            Error::InvalidVectorDimension { current, expected }
        );
        Ok(())
    }
