...
mod tests {
    ...
    async fn check_get_doc_key_id(ctx: &FrozenContext, d: &SeqDocIds, doc_id: DocId, key: &str) {
        ...
        assert_eq!(
            SeqDocIds::get_id(&d.ikb, &tx, doc_id).await.unwrap(),
            Some(id.clone())
        );
    ...
    async fn test_resolve_doc_id() {
        ...
        // Resolve a first doc key
        {
            ...
            let doc_id = d
                .resolve_doc_id(&ctx, "Foo".to_owned().into())
                .await
                .unwrap();
        ...
        // Resolve the same doc key
        {
            ...
            let doc_id = d
                .resolve_doc_id(&tx, "Foo".to_owned().into())
                .await
                .unwrap();
        ...
        // Resolve another single doc key
        {
            let (tx, d) = new_operation(&ds, Write).await;
            let doc_id = d
                .resolve_doc_id(&tx, "Bar".to_owned().into())
                .await
                .unwrap();
        ...
        // Resolve another two existing doc keys and two new doc keys (interlaced)
        {
            let (tx, d) = new_operation(&ds, Write).await;
            assert_eq!(
                d.resolve_doc_id(&tx, "Foo".to_owned().into())
                    .await
                    .unwrap(),
                Resolved::Existing(0)
            );
            assert_eq!(
                d.resolve_doc_id(&tx, "Hello".to_owned().into())
                    .await
                    .unwrap(),
                Resolved::New(2)
            );
            assert_eq!(
                d.resolve_doc_id(&tx, "Bar".to_owned().into())
                    .await
                    .unwrap(),
                Resolved::Existing(1)
            );
            assert_eq!(
                d.resolve_doc_id(&tx, "World".to_owned().into())
                    .await
                    .unwrap(),
                Resolved::New(3)
            );
    ...
    async fn test_remove_doc_id() {
        ...
        // Check 'Foo' has been removed
        {
            let (ctx, d) = new_operation(&ds, Read).await;
            assert_eq!(
                d.get_doc_id(&ctx.tx(), &"Foo".to_owned().into())
                    .await
                    .unwrap(),
                None
            );
            assert_eq!(
                d.get_doc_id(&ctx.tx(), &"Bar".to_owned().into())
                    .await
                    .unwrap(),
                Some(1)
            );
        ...
        // Insert a new doc - should take the next available id 2
        {
            let (ctx, d) = new_operation(&ds, Write).await;
            assert_eq!(
                d.resolve_doc_id(&ctx, "Hello".to_owned().into())
                    .await
                    .unwrap(),
                Resolved::New(2)
            );
        ...
        // Check we have "Hello" and "Bar"
        {
            ...
            assert_eq!(
                d.get_doc_id(&ctx.tx(), &"Hello".to_owned().into())
                    .await
                    .unwrap(),
                Some(2)
            );
        ...
        // Insert a new doc - should take the available id 3
        {
            let (ctx, d) = new_operation(&ds, Write).await;
            assert_eq!(
                d.resolve_doc_id(&ctx, "World".to_owned().into())
                    .await
                    .unwrap(),
                Resolved::New(3)
            );
        ...
        // Check "World" has been added
        {
            ...
            assert_eq!(
                d.get_doc_id(&ctx.tx(), &"World".to_owned().into())
                    .await
                    .unwrap(),
                Some(3)
            );
