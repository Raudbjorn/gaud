...
impl PlanBuilder {
    ...
    fn check_compound_index_all_and(
        ...
        if equal_combinations.len() == 1 {
            let equals: Vec<Value> = equal_combinations[0]
                .iter()
    ...
    pub(crate) fn explain(&self) -> Value {
        ...
        match self.op() {
            ...
            IndexOperator::Matches(qs, op) => {
                e.insert(
                    "operator",
                    Value::from(BinaryOperator::Matches(op.clone()).to_sql()),
                );
            ...
            IndexOperator::Order(reverse) => {
                e.insert(
                    "operator",
                    Value::from(if *reverse { "ReverseOrder" } else { "Order" }),
                );
            }
            IndexOperator::Count => {
...
impl UnionRangeQueryBuilder {
    ...
    fn new(exp: Arc<Expr>, io: IndexOption) -> Option<Self> {
        let mut b = Self::default();
        if b.add(exp, io) { Some(b) } else { None }
    }
