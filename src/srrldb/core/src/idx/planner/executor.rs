            ...
            match &index_reference.index {
                Index::FullText(p) => {
                    ...
                    if let Some(e) = fulltext_entry {
                        if let Matches(_, MatchesOperator { rf: Some(mr), .. }) = e.0.io.op() {
                            let mr_entry = PerMatchRefEntry::FullText(e.clone());
                            ensure!(
                                mr_entries.insert(*mr, mr_entry).is_none(),
                                Error::DuplicatedMatchRef { mr: *mr }
                            );
                        }
...
impl QueryExecutor {
    ...
    async fn new_single_iterator(
        ...
        match io.index_reference().index {
            ...
            Index::Uniq => Ok(self
                .new_unique_index_iterator(ns, db, irf, io.clone())
                .await?),
            Index::FullText { .. } => self.new_fulltext_index_iterator(irf, io.clone()).await,
        ...
        Ok(match io.op() {
            ...
            IndexOperator::Order(reverse) => {
                if *reverse {
                    ...
                    Some(RecordIterator::IndexRange(
                        IndexRangeThingIterator::full_range(ir, ns, db, ix)?,
                    ))
                }
            }
    ...
    fn new_index_equal_iterator(
        ...
        Ok(RecordIterator::IndexEqual(IndexEqualThingIterator::new(
            irf, ns, db, ix, fd,
        )?))
    }
        ...
        Ok(match io.op() {
            ...
            IndexOperator::Order(reverse) => {
                if *reverse {
                    ...
                    Some(RecordIterator::UniqueRange(
                        UniqueRangeThingIterator::full_range(irf, ns, db, io.index_reference())?,
                    ))
                }
            }
            IndexOperator::Range(prefix, ranges) => Some(RecordIterator::UniqueRange(
                UniqueRangeThingIterator::compound_range(
                    irf,
                    ns,
                    db,
                    io.index_reference(),
    ...
    fn new_unique_equal_iterator(
        ...
        if ix.cols.len() > 1 {
            ...
            Ok(RecordIterator::UniqueEqual(UniqueEqualThingIterator::new(
                irf, ns, db, ix, fd,
            )?))
        }
    }
    ...
    async fn new_fulltext_index_iterator(
        ...
        if let Some(IteratorEntry::Single(Some(exp), ..)) = self.0.it_entries.get(ir)
            && let Matches(_, MatchesOperator { operator, .. }) = io.op()
    ...
    pub(crate) async fn matches(
        ...
        if let Some(PerExpressionEntry::FullText(fte)) = self.0.exp_entries.get(exp) {
            ...
            if let Some(PerIndexReferenceIndex::FullText(fti)) = self.0.ir_map.get(ix) {
                ...
                return self
                    .fulltext_matches_with_value(stk, ctx, opt, fti, fte, l, r)
                    .await;
            }
        }
    ...
    async fn fulltext_matches_with_value(
        ...
        fti.matches_value(stk, ctx, opt, &fte.0.qt, fte.0.bo, v)
            .await
    }
    ...
    pub(crate) async fn score(
        ...
        if let Some(mre) = self.get_match_ref_entry(match_ref) {
            let mut doc_id = if let Some(ir) = ir { ir.doc_id() } else { None };
            match mre {
                PerMatchRefEntry::FullText(fte) => {
...
impl FullTextEntry {
    async fn new(
        ...
        if let Matches(qs, mo) = io.op() {
            let qt = fti
                .extract_querying_terms(stk, ctx, opt, qs.to_owned())
                .await?;
