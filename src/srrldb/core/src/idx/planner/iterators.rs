        ...
        match self {
            ...
            Self::IndexCount(_) => {
                bail!(Error::unreachable(
                    "IndexCount should not be used with next_batch"
                ))
            }
        }
    }
...
impl IndexEqualThingIterator {
    ...
    async fn next_scan_batch<B: IteratorBatch>(
        ...
        res.into_iter().try_for_each(|(_, val)| -> Result<()> {
            records.add(IndexItemRecord::new_key(
                revision::from_slice(&val)?,
                irf.into(),
            ));
            Ok(())
        })?;
    ...
    async fn next_count(&mut self, tx: &Transaction, limit: u32) -> Result<usize> {
        Ok(Self::next_scan(tx, &mut self.beg, &self.end, limit)
            .await?
            .len())
    }
}
...
impl IndexRangeThingIterator {
    ...
    fn reduce_range(ranges: &[(BinaryOperator, Arc<Value>)]) -> Result<(RangeValue, RangeValue)> {
        ...
        let cmp = |(a1, a2, _): &(Vec<u8>, bool, Arc<Value>),
                   (b1, b2, _): &(Vec<u8>, bool, Arc<Value>)| {
...
impl IndexRangeReverseThingIterator {
    ...
    async fn check_batch_ending(
        ...
        if let Some(v) = tx.get(&self.r.r.end, None).await? {
            *limit -= 1;
            Ok(Some(IndexItemRecord::new_key(
                revision::from_slice(&v)?,
                self.irf.into(),
            )))
        } else {
...
impl IndexJoinThingIterator {
    pub(super) fn new(
        ...
        Ok(Self(
            irf,
            JoinThingIterator::new(ns, db, ix, remote_iterators)?,
        ))
    }
