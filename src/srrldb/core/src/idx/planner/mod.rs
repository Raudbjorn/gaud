...
impl<'a> StatementContext<'a> {
    ...
    pub(crate) fn check_record_strategy(
        ...
        if matches!(
            self.stm,
            Statement::Update(_) | Statement::Upsert(_) | Statement::Delete(_)
        ) {
...
impl QueryPlanner {
    ...
    pub(crate) async fn next_iteration_stage(&self) -> Option<IterationStage> {
        ...
        match self.iteration_workflow.get(pos as usize) {
            Some(IterationStage::BuildKnn) => Some(IterationStage::Iterate(Some(
                self.build_bruteforce_knn_results().await,
            ))),
            is => is.cloned(),
        }
    }
