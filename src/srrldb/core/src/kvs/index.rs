...
impl Building {
    ...
    async fn new_read_tx(&self) -> Result<Transaction> {
        self.tf
            .transaction(
                ...
                Optimistic,
                self.ctx.try_get_sequences()?.clone(),
            )
            .await
    }
    ...
    async fn check_prepare_remove(&self, last_decommissioned_check: &mut Instant) -> Result<()> {
        let tx = self.new_read_tx().await?;
            ...
            tx,
            self.check_prepare_remove_with_tx(last_decommissioned_check, &tx)
                .await
        );
        tx.cancel().await?;
        Ok(())
    ...
    async fn run(&self) -> Result<()> {
        ...
        while let Some(rng) = next {
            ...
            let batch = {
                ...
                let res = catch!(
                    tx,
                    tx.batch_keys_vals(rng, *INDEXING_BATCH_SIZE, None).await
                );
                tx.cancel().await?;
                res
            };
            ...
            // Create a new context with a "write" transaction
            {
                ...
                // Index the batch
                    ...
                    tx,
                    self.index_initial_batch(&ctx, &tx, batch.result, &mut initial_count)
                        .await
                );
                catch!(tx, tx.commit().await);
            }
        ...
        loop {
            ...
            self.is_beyond_threshold(None)?;
            // Check the index still exists and is not decommissioned
            self.check_prepare_remove(&mut last_prepare_remove_check)
                .await?;
            let range = {
                let mut queue = self.queue.write().await;
