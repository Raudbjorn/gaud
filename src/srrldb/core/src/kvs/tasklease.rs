    ...
    pub(super) async fn has_lease(&self) -> Result<bool> {
        ...
        while tempo < MAX_BACKOFF {
            match self.check_lease(Utc::now()).await {
                ...
                Err(e) => {
                    trace!(
                        "Tolerated error while getting a lease for {:?}: {e}",
                        self.task_type
                    );
                }
            }
...
mod tests {
    ...
    async fn task_lease_concurrency_memory() {
        ...
        let flavor = crate::kvs::mem::Datastore::new()
            .await
    ...
    async fn test_lease_renewal_behavior() {
        ...
        assert!(
            has_lease,
            "Should still have the lease without re-acquiring"
        );
        ...
        assert!(
            has_lease,
            "Should still have the lease after attempted renewal"
        );
