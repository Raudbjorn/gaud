...
#[allow(
    dead_code,
    reason = "Not used when none of the storage backends are enabled."
)]
...
pub trait Transactable: requirements::TransactionRequirements {
    ...
    async fn getr(&self, rng: Range<Key>, version: Option<u64>) -> Result<Vec<(Key, Val)>> {
        ...
        while let Some(rng) = next {
            let res = self
                .batch_keys_vals(rng, *NORMAL_FETCH_SIZE, version)
                .await?;
            next = res.next;
            for v in res.result {
                out.push(v);
        ...
        if res.len() < batch as usize && batch > 0 {
            ...
            match res.last() {
                Some(k) => {
                    ...
                    Ok(Batch::<Key>::new(Some(Range { start: k, end }), res))
                }
        ...
        if res.len() < batch as usize && batch > 0 {
            ...
            match res.last() {
                Some((k, _)) => {
                    let mut k = k.clone();
                    util::advance_key(&mut k);
                    Ok(Batch::<(Key, Val)>::new(Some(Range { start: k, end }), res))
                }
                // We have checked the length above, so
                // there should be a last item in the
                // vector, so we shouldn't arrive here
