...
impl Transaction {
    ...
    pub async fn export(
        ...
        self.export_metadata(&cfg, &chn, db.namespace_id, db.database_id)
            .await?;
        // Output TABLES
        self.export_tables(&cfg, &chn, db.namespace_id, db.database_id)
            .await?;
        Ok(())
    }
    ...
    async fn export_metadata(
        ...
        // Output OPTIONS
        self.export_section("OPTION", [OptionStatement::import()].into_iter(), chn)
            .await?;
        ...
        if cfg.users {
            ...
            self.export_section(
                ...
                users
                    .iter()
                    .map(|x| DefineUserStatement::from_definition(Base::Db, x)),
                chn,
            )
            .await?;
        }
        ...
        if cfg.functions {
            ...
            self.export_section("FUNCTIONS", functions.iter(), chn)
                .await?;
        }
        ...
        if cfg.analyzers {
            ...
            self.export_section(
                ...
                analyzers
                    .iter()
                    .map(DefineAnalyzerStatement::from_definition),
                chn,
            )
            .await?;
        }
        ...
        if cfg.sequences {
            ...
            self.export_section("SEQUENCES", sequences.iter(), chn)
                .await?;
        }
        ...
        T: ToSql,
    {
        ...
        chn.send(bytes!("-- ------------------------------"))
            .await?;
        chn.send(bytes!(format!("-- {}", InlineCommentDisplay(title))))
            .await?;
        chn.send(bytes!("-- ------------------------------"))
            .await?;
