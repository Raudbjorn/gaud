...
impl HlcTimestamp {
    ...
    pub fn next() -> Self {
        ...
        loop {
            ...
            if LAST_TIMESTAMP
                .compare_exchange_weak(last, next, ordering, ordering)
                .is_ok()
            {
...
mod tests {
    ...
    fn test_u64_bytes_big_endian() {
        ...
        assert!(
            ...
            "Bytes should be lexicographically ordered"
        );
    }
    ...
    fn test_u64_versionstamp_roundtrip() {
        ...
        for &value in &values {
            ...
            assert_eq!(
                value, recovered,
                "Failed versionstamp roundtrip for u64 value {}",
                value
            );
        }
    }
    ...
    fn test_hlc_bytes_lexicographic_ordering() {
        ...
        assert!(
            bytes1 < bytes2,
            "Same millisecond, counter should order lexicographically"
        );
        assert!(
            bytes2 < bytes3,
            "Different milliseconds should order lexicographically"
        );
    }
    ...
    fn test_hlc_versionstamp_roundtrip() {
        ...
        for value in test_values {
            ...
            assert_eq!(
                ts, recovered,
                "Failed versionstamp roundtrip for HLC value {}",
                value
            );
        }
    }
