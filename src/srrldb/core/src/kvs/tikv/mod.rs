...
impl Timestamp for Timecode {
    ...
    fn from_ts_bytes(bytes: &[u8]) -> Result<Self> {
        match bytes.try_into() {
            Ok(v) => Ok(Timecode(tikv::Timestamp::from_version(u64::from_be_bytes(
                v,
            )))),
            Err(_) => Err(Error::TimestampInvalid(
                "timestamp should be 8 bytes".to_string(),
            )),
        }
    }
}
...
impl Datastore {
    ...
    pub(crate) async fn new(path: &str) -> Result<Datastore> {
        ...
        match client.await {
            Ok(db) => Ok(Datastore { db: Arc::pin(db) }),
...
impl Transactable for Transaction {
        ...
        if !inner.savepoints.is_empty() || !inner.operations.is_empty() {
            match old_val {
                Some(existing_val) => {
                    ...
                    inner
                        .operations
                        .push(Operation::RestoreValue(key, existing_val));
                }
                None => {
    ...
    async fn del(&self, key: Key) -> Result<()> {
        ...
        if let Some(existing_val) = old_val {
            ...
            inner
                .operations
                .push(Operation::RestoreDeleted(key, existing_val));
        }
        // Return result
        Ok(())
    }
    ...
    async fn new_save_point(&self) -> Result<()> {
        ...
        inner.savepoints.push(Savepoint { operations });
        // Continue
        Ok(())
    }
    ...
    async fn rollback_to_save_point(&self) -> Result<()> {
        ...
        if inner.savepoints.is_empty() {
            return Err(Error::Transaction(
                "No savepoint to rollback to".to_string(),
            ));
        }
    ...
    async fn timestamp(&self) -> Result<Box<dyn Timestamp>> {
        Ok(Box::new(Timecode(
            self.inner.write().await.tx.current_timestamp().await?,
        )))
    }
