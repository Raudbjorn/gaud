...
impl Transaction {
        ...
        K: KVKey + Debug,
    {
        ...
        Ok(self
            .tr
            .batch_keys(beg..end, batch, version)
            .await
            .map_err(Error::from)?)
    }
        ...
        K: KVKey + Debug,
    {
        ...
        Ok(self
            .tr
            .batch_keys_vals(beg..end, batch, version)
            .await
            .map_err(Error::from)?)
    }
        ...
        K: KVKey + Debug,
    {
        ...
        Ok(self
            .tr
            .batch_keys_vals_versions(beg..end, batch)
            .await
            .map_err(Error::from)?)
    }
    ...
    pub async fn timestamp_bytes_from_versionstamp(&self, version: u128) -> Result<Vec<u8>> {
        Ok(self
            .tr
            .timestamp_bytes_from_versionstamp(version)
            .await
            .map_err(Error::from)?)
    }
    ...
    pub async fn timestamp_bytes_from_datetime(&self, datetime: DateTime<Utc>) -> Result<Vec<u8>> {
        Ok(self
            .tr
            .timestamp_bytes_from_datetime(datetime)
            .await
            .map_err(Error::from)?)
    }
    ...
    fn set_record_cache(
        ...
        self.cache
            .insert(lookup.to_owned_key(), Arc::clone(&record));
    }
...
impl NamespaceProvider for Transaction {
    ...
    async fn put_ns(&self, ns: NamespaceDefinition) -> Result<Arc<NamespaceDefinition>> {
        ...
        self.cache
            .insert(lookup.to_owned_key(), Arc::clone(&cached_ns));
...
impl DatabaseProvider for Transaction {
    ...
    async fn put_db(&self, ns: &str, db: DatabaseDefinition) -> Result<Arc<DatabaseDefinition>> {
        ...
        self.cache
            .insert(lookup.to_owned_key(), Arc::clone(&cached_db));
...
impl TableProvider for Transaction {
    ...
    async fn all_tb_views(
        ...
        let lookup = cache::tx::key::TableViewsCacheKeyRef(ns, db, tb);
        match self.cache.get_clone_by(&lookup) {
            Some(val) => Ok(val),
            None => {
    ...
    async fn get_or_add_tb(
        ...
        let lookup = cache::tx::key::TableByNameCacheKeyRef(ns, db, tb);
        match self.cache.get_clone_by(&lookup) {
            // The entry is in the cache
            Some(val) => Ok(val),
            // The entry is not in the cache
            None => {
                ...
                if let Some(tb_def) = self.get(&table_key, None).await? {
                    ...
                    self.cache
                        .insert(lookup.to_owned_key(), Arc::clone(&cached_tb));
                    return Ok(cached_tb);
                }
    ...
    async fn del_tb(&self, ns: &str, db: &str, tb: &TableName) -> Result<()> {
        let Some(tb) = self.get_tb_by_name(ns, db, tb).await? else {
            return Err(Error::TbNotFound { name: tb.clone() }.into());
        };
    ...
    async fn all_tb_fields(
        ...
        let lookup = cache::tx::key::TableFieldsCacheKeyRef(ns, db, tb);
        match self.cache.get_clone_by(&lookup) {
            Some(val) => Ok(val),
            None => {
    ...
    async fn all_tb_indexes(
        ...
        let lookup = cache::tx::key::TableIndexesCacheKeyRef(ns, db, tb);
        match self.cache.get_clone_by(&lookup) {
            Some(val) => Ok(val),
            None => {
    ...
    async fn all_tb_lives(
        ...
        let lookup = cache::tx::key::TableLivesCacheKeyRef(ns, db, tb);
        match self.cache.get_clone_by(&lookup) {
            Some(val) => Ok(val),
            None => {
    ...
    async fn get_tb(
        ...
        let lookup = cache::tx::key::TableCacheKeyRef(ns, db, tb);
        match self.cache.get_clone_by(&lookup) {
            Some(val) => Ok(Some(val)),
            None => {
    ...
    async fn put_tb_index(
        ...
        self.cache
            .insert(lookup.to_owned_key(), Arc::new(ix.clone()));
        Ok(())
    }
    ...
    async fn get_db_bucket(
        ...
        match self.cache.get_clone_by(&lookup) {
            ...
            None => {
                ...
                self.cache
                    .insert(lookup.to_owned_key(), Arc::clone(&bucket_def));
                Ok(Some(bucket_def))
            }
        }
    }
