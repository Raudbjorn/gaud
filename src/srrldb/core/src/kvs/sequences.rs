...
impl Sequences {
    ...
    pub(crate) async fn next_user_sequence_id(
        ...
        self.next_val(ctx, domain, seq.start, seq.batch, seq.timeout)
            .await
    }
...
impl Sequence {
    ...
    async fn next(
        ...
        let tx = self
            .tf
            .transaction(TransactionType::Write, LockType::Optimistic, sqs.clone())
            .await?;

        // Execute operations and ensure transaction is cancelled on error
        match tx
            .set(&self.state_key, &revision::to_vec(&self.st)?, None)
            .await
        {
            Ok(_) => {
                tx.commit().await?;
