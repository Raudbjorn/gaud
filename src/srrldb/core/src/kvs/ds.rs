    ...
    async fn new_transaction_builder(
        ...
        let (flavour, path) = match path.split_once("://").or_else(|| path.split_once(':')) {
            ...
            _ => bail!(Error::Unreachable(
                "Provide a valid database path parameter".to_owned()
            )),
        };
        ...
        match (flavour, path) {
            ...
            (flavour @ "indxdb", path) => {
                #[cfg(feature = "kv-indxdb")]
                {
                    let v = super::indxdb::Datastore::new(&path)
                        .await
                        .map(DatastoreFlavor::IndxDB)?;
                    let c = clock.unwrap_or_else(|| Arc::new(SizedClock::system()));
                    info!(target: TARGET, "Started {flavour} kvs store");
                    Ok((Box::<DatastoreFlavor>::new(v), c))
                }
            ...
            (flavour @ "tikv", path) => {
                #[cfg(feature = "kv-tikv")]
                {
                    let v = super::tikv::Datastore::new(&path)
                        .await
...
impl Datastore {
    ...
    >(
        ...
        // Initiate the desired datastore
        let (builder, clock) = composer
            .new_transaction_builder(path, clock, canceller)
            .await?;
        //
    ...
    pub async fn initialise_defaults(&self, namespace: &str, database: &str) -> Result<()> {
        ...
        self.execute(&sql, &Session::owner(), Some(vars.into()))
            .await?;
        // Everything ok
        Ok(())
    }
        ...
        for id in archived.iter() {
            ...
            let txn = self.transaction(Write, Optimistic).await?;
            {
                ...
                while let Some(rng) = next {
                    ...
                    let res = catch!(
                        txn,
                        txn.batch_keys_vals(rng, *NORMAL_FETCH_SIZE, None).await
                    );
                    next = res.next;
    ...
    pub async fn index_compaction(&self, interval: Duration) -> Result<()> {
        ...
        loop {
            ...
            for (k, _) in items {
                ...
                match catch!(
                    txn,
                    txn.get_tb_index_by_id(ic.ns, ic.db, ic.tb.as_ref(), ic.ix)
                        .await
                ) {
    ...
    pub async fn transaction(&self, write: TransactionType, lock: LockType) -> Result<Transaction> {
        self.transaction_factory
            .transaction(write, lock, self.sequences.clone())
            .await
    }
    ...
    pub async fn process_with_transaction(
        ...
        // Check if the session has expired
        if sess.expired() {
            return Err(DbResultError::InvalidAuth(
        ...
        if let Err(e) = self.check_anon(sess) {
            return Err(DbResultError::InvalidAuth(format!(
    ...
    pub(crate) async fn process_plan(
        ...
        let mut ctx = self
            ...
            .map_err(|e| match e.downcast_ref::<Error>() {
                ...
                Some(Error::QueryNotExecuted { message }) => {
        ...
        // Process all statements
        Executor::execute_plan(self, ctx.freeze(), opt, plan)
            .await
            .map_err(|e| match e.downcast_ref::<Error>() {
                Some(Error::ExpiredSession) => {
                    DbResultError::InvalidAuth("The session has expired".to_string())
    ...
    pub async fn get_db_model(
        ...
        let model = tx
            .get_db_model(db.namespace_id, db.database_id, model_name, model_version)
            .await?;
        tx.cancel().await?;
        Ok(model)
    }
    ...
    pub async fn invoke_api_handler(
        ...
        let tx = Arc::new(
            self.transaction(TransactionType::Write, LockType::Optimistic)
                .await?,
        );
    ...
    pub async fn put_ml_model(
        ...
        let q = LogicalPlan {
            expressions: vec![TopLevelExpr::Expr(Expr::Define(Box::new(
                DefineStatement::Model(model),
            )))],
        };

        self.process_plan(q, session, None)
            .await
            .map_err(|e| anyhow::anyhow!(e))?;
...
mod test {
    ...
    async fn test_setup_superuser() {
        ...
        ds.initialise_credentials(username, password).await.unwrap();
        {
            ...
            assert_eq!(
                pass_hash,
                txn.expect_root_user(username).await.unwrap().hash.clone()
            );
            txn.cancel().await.unwrap();
        }
    }
    ...
    pub async fn very_deep_query() -> Result<()> {
        ...
        // Start a new transaction
        let txn = dbs
            .transaction(TransactionType::Read, Optimistic)
            .await?
            .enclose();
        // Store the transaction
        ctx.set_transaction(txn.clone());
    ...
    async fn cross_transaction_caching_uuids_updated() -> Result<()> {
        ...
        let db = {
            let txn = ds
                .transaction(TransactionType::Write, LockType::Pessimistic)
                .await?;
        ...
        let (initial, initial_live_query_version) = {
            ...
            let txn = ds
                .transaction(TransactionType::Read, LockType::Pessimistic)
                .await?;
            let tb = TableName::from("test");
            let initial = txn
                .get_tb(db.namespace_id, db.database_id, &tb)
                .await?
                .unwrap();
	...
	"
        ...
        // Obtain the uuids after definitions
        {
            ...
            assert_ne!(
                after_define_live_query_version,
                after_remove_live_query_version
            );
        }
        //
        Ok(())
