...
pub async fn multiwriter_same_keys_conflict(new_ds: impl CreateDs) {
    ...
    tx.set(&"test", &"some text".as_bytes().to_vec(), None)
        .await
        .unwrap();
    tx.commit().await.unwrap();
    // Create a writeable transaction
    let tx1 = ds.transaction(Write, Optimistic).await.unwrap();
    tx1.set(&"test", &"other text 1".as_bytes().to_vec(), None)
        .await
        .unwrap();
    // Create a writeable transaction
    let tx2 = ds.transaction(Write, Optimistic).await.unwrap();
    tx2.set(&"test", &"other text 2".as_bytes().to_vec(), None)
        .await
        .unwrap();
    // Create a writeable transaction
    let tx3 = ds.transaction(Write, Optimistic).await.unwrap();
    tx3.set(&"test", &"other text 3".as_bytes().to_vec(), None)
        .await
        .unwrap();
    ...
    // Create a writeable transaction
    let tx = ds.transaction(Write, Optimistic).await.unwrap();
    tx.set(&"test", &"original text".as_bytes().to_vec(), None)
        .await
        .unwrap();
    tx.commit().await.unwrap();
    // Check that the key was updated ok
