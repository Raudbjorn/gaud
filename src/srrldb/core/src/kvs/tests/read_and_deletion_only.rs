...
pub async fn read_and_deletion_only(new_ds: impl CreateDs) {
    ...
    // Phase 1: Initial writes in normal mode (before reaching space limit)
    {
        ...
        tx.set(&"initial_key", &"initial_value".as_bytes().to_vec(), None)
            .await
            .unwrap();
    ...
    ongoing_tx
        .set(&"ongoing_key", &"ongoing_value".as_bytes().to_vec(), None)
        .await
        .unwrap();
    ...
    // Confirm new write transactions are blocked
    {
        let tx = ds.transaction(Write, Optimistic).await.unwrap();
        let res = tx
            .put(&"other_key", &"other_value".as_bytes().to_vec(), None)
            .await;
        assert!(
            res.unwrap_err()
                .to_string()
                .contains("read-and-deletion-only mode"),
    ...
    {
        let tx = ds.transaction(Write, Optimistic).await.unwrap();
        tx.put(&"other_key", &"other_value".as_bytes().to_vec(), None)
            .await
            .unwrap();
        tx.commit().await.unwrap();
    }
