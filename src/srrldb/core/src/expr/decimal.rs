...
impl DecimalLexEncoder {
    ...
    pub(crate) fn decode(bytes: &[u8]) -> Result<D128> {
        ...
        Ok(D128::from_parts(
            mantissa,
            exponent,
            if is_negative { Sign::Minus } else { Sign::Plus },
            Context::default(),
        ))
    }
...
mod tests {
    ...
    fn test_lexicographic_ordering() {
        ...
        for (i, window) in cases.windows(2).enumerate() {
            ...
            assert!(
                b1 < b2,
                "#{i} - {n1:?} < {n2:?} (after serialization) - {b1:?} < {b2:?}"
            );
        }
    }
    ...
    fn test_decode_empty_buffer() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("Cannot decode from empty buffer")
        );
    }
    ...
    fn test_decode_invalid_marker() {
        ...
        assert_eq!(
            err.to_string(),
            "Serialization error: Invalid marker byte: 66",
            "{err:?}"
        );
    }
