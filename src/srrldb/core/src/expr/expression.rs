...
impl Expr {
    ...
    pub(crate) fn read_only(&self) -> bool {
        match self {
            ...
            Expr::Prefix { expr, .. } | Expr::Postfix { expr, .. } => expr.read_only(),
            Expr::Binary { left, right, .. } => left.read_only() && right.read_only(),
    ...
    pub fn from_public_value(value: PublicValue) -> Self {
        match value {
            ...
            srrldb_types::Value::Array(a) => Expr::Literal(Literal::Array(
                a.into_iter().map(Expr::from_public_value).collect(),
            )),
            srrldb_types::Value::Set(s) => Expr::Literal(Literal::Array(
                s.into_iter().map(Expr::from_public_value).collect(),
            )),
            ...
            srrldb_types::Value::RecordId(RecordId { table, key }) => {
...
impl Expr {
    ...
    pub(crate) fn is_static(&self) -> bool {
        match self {
            ...
            Expr::Prefix { expr, .. } => expr.is_static(),
            Expr::Postfix { expr, .. } => expr.is_static(),
            Expr::Binary { left, right, .. } => left.is_static() && right.is_static(),
            Expr::FunctionCall(x) => {
    ...
    pub(crate) async fn compute(
        ...
        match self {
            ...
            Expr::Prefix { op, expr } => Self::compute_prefix(stk, ctx, &opt, doc, op, expr).await,
            Expr::Postfix { expr, op } => {
                Self::compute_postfix(stk, ctx, &opt, doc, expr, op).await
            }
            Expr::Binary { .. } => Self::compute_binary(stk, ctx, &opt, doc, self).await,
            ...
            Expr::Throw(expr) => {
                ...
                Err(ControlFlow::Err(anyhow::Error::new(Error::Thrown(
                    res.to_raw_string(),
                ))))
            }
            Expr::IfElse(ifelse_statement) => ifelse_statement.compute(stk, ctx, &opt, doc).await,
            Expr::Select(select_statement) => select_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            Expr::Create(create_statement) => create_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            Expr::Update(update_statement) => update_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            Expr::Delete(delete_statement) => delete_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            Expr::Relate(relate_statement) => relate_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            Expr::Insert(insert_statement) => insert_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            Expr::Define(define_statement) => define_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            Expr::Remove(remove_statement) => remove_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            Expr::Rebuild(rebuild_statement) => rebuild_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            Expr::Upsert(upsert_statement) => upsert_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            Expr::Alter(alter_statement) => alter_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            Expr::Info(info_statement) => info_statement
                .compute(stk, ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
            ...
            Expr::Sleep(sleep_statement) => sleep_statement
                .compute(ctx, &opt, doc)
                .await
                .map_err(ControlFlow::Err),
        }
    ...
    async fn compute_postfix(
        ...
        match op {
            ...
            PostfixOperator::MethodCall(name, exprs) => {
                ...
                {
                    return x
                        .invoke(stk, ctx, opt, doc, args)
                        .await
                        .map_err(ControlFlow::Err);
                };
                fnc::idiom(stk, ctx, opt, doc, res, name, args)
                    .await
                    .map_err(ControlFlow::Err)
            }
    ...
    async fn compute_binary(
        ...
        if let BinaryOperator::NearestNeighbor(_) = op {
            return fnc::operate::knn(stk, ctx, opt, doc, expr)
                .await
                .map_err(ControlFlow::Err);
        }
