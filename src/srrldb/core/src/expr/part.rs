...
impl Part {
    ...
    pub(crate) fn is_index(&self) -> bool {
        matches!(
            self,
            Part::Value(Expr::Literal(Literal::Integer(_))) | Part::First | Part::Last
        )
    }
                ...
                for (j, p) in parts.iter().enumerate() {
                    let plan = match p {
                        ...
                        DestructurePart::Destructure(field, parts) => {
                            Part::Destructure(parts.to_owned())
                                .recursion_plan()
                                .map(|plan| {
                                    (
                                        field.to_owned(),
    ...
    pub async fn compute(
        ...
        match rec.current {
            Value::Array(value) => stk
                ...
                .map(Into::into),
            _ => {
                stk.run(|stk| self.compute_inner(stk, ctx, opt, doc, rec))
                    .await
            }
        }
    }
        ...
        match self {
            ...
            Self::Destructure {
                ...
                let v = stk
                    ...
                    .catch_return()?;
                let v = plan
                    .compute(stk, ctx, opt, doc, rec.with_current(&v))
                    .await?;
                let v = stk
                    .run(|stk| v.get(stk, ctx, opt, doc, after))
                    .await
                    .catch_return()?;
...
async fn walk_paths(
    ...
    for path in paths.iter() {
        ...
        let res = stk
            .run(|stk| last.get(stk, ctx, opt, doc, recursion.path))
            .await
            .catch_return()?;
