...
impl RelateStatement {
    ...
    pub(crate) async fn compute(
        ...
        let from = {
            let mut out = Vec::new();
            match stk
                .run(|stk| self.from.compute(stk, ctx, opt, doc))
                .await
                .catch_return()?
            {
                    ...
                    for v in v {
                        match v {
                            ...
                            Value::Object(v) => match v.rid() {
                                Some(v) => out.push(v),
                                _ => {
                                    bail!(Error::RelateStatementIn { value: v.to_sql() })
                                }
                            },
                            v => {
                                bail!(Error::RelateStatementIn { value: v.to_sql() })
        ...
        let to = {
            let mut out = Vec::new();
            match stk
                .run(|stk| self.to.compute(stk, ctx, opt, doc))
                .await
                .catch_return()?
            {
                Value::RecordId(v) => out.push(v),
                    ...
                    for v in v {
                        match v {
                            ...
                            Value::Object(v) => match v.rid() {
                                Some(v) => out.push(v),
                                None => {
                                    bail!(Error::RelateStatementId { value: v.to_sql() })
                                }
                            },
                            v => {
                                bail!(Error::RelateStatementId { value: v.to_sql() })
        ...
        for f in from.iter() {
            for t in to.iter() {
                ...
                let tb = txn
                    .get_or_add_tb(Some(ctx), opt.ns()?, opt.db()?, through_table)
                    .await?;
                let fields = txn
                ...
                iterator.ingest(Iterable::Relatable(
                    doc_ctx,
                    f.clone(),
                    through,
                    t.clone(),
