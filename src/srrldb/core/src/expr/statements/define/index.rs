...
impl DefineIndexStatement {
    ...
    pub(crate) async fn compute(
        ...
        let index_id = if let Some(ix) = txn
            .get_tb_index(tb.namespace_id, tb.database_id, &tb.name, &name)
            .await?
        {
            match self.kind {
                DefineKind::Default => {
        ...
        txn.put_tb_index(tb.namespace_id, tb.database_id, &tb.name, &index_def)
            .await?;
