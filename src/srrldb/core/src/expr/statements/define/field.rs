...
impl DefineFieldStatement {
    pub(crate) async fn to_definition(
        ...
        Ok(catalog::FieldDefinition {
            ...
            table: expr_to_ident(stk, ctx, opt, doc, &self.what, "table name")
                .await?
                .into(),
    ...
    pub(crate) async fn compute(
        ...
        self.validate_computed_options(ns, db, ctx.tx(), &definition)
            .await?;
        ...
        self.disallow_mismatched_types(ctx, ns, db, &definition)
            .await?;
        ...
        self.validate_flexible_restrictions(ctx, ns, db, &definition)
            .await?;
        ...
        let tb = txn
            .get_or_add_tb(Some(ctx), ns_name, db_name, &definition.table)
            .await?;
        ...
        self.process_recursive_definitions(ns, db, txn.clone(), &definition)
            .await?;
    ...
    pub(crate) async fn process_recursive_definitions(
        ...
        let fields = txn
            .all_tb_fields(ns, db, &definition.table, None)
            .await
            .ok();
        ...
        if let Some(mut cur_kind) = self.field_kind.as_ref().and_then(|x| x.inner_kind()) {
            ...
            loop {
                ...
                let val = if let Some(existing) = fields
                    .as_ref()
    ...
    pub(crate) async fn validate_computed_options(
        ...
        if self.computed.is_some() {
            // Ensure the field is not the `id` field
                ...
                !definition.name.is_id(),
                Error::IdFieldKeywordConflict("COMPUTED".into())
            );
            ...
            // Ensure there are no conflicting clauses
                ...
                self.value.is_none(),
                Error::ComputedKeywordConflict("VALUE".into())
            );
            ensure!(
                self.assert.is_none(),
                Error::ComputedKeywordConflict("ASSERT".into())
            );
            ensure!(
                self.reference.is_none(),
                Error::ComputedKeywordConflict("REFERENCE".into())
            );
            ensure!(
                ...
                !self.readonly,
                Error::ComputedKeywordConflict("READONLY".into())
            );
    ...
    pub(crate) fn validate_id_restrictions(
        ...
        if definition.name.is_id() {
            // Ensure no `VALUE` clause is specified
                ...
                self.value.is_none(),
                Error::IdFieldKeywordConflict("VALUE".into())
            );

            // Ensure no `REFERENCE` clause is specified
            ensure!(
                self.reference.is_none(),
                Error::IdFieldKeywordConflict("REFERENCE".into())
            );
