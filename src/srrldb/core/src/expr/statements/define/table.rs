...
impl DefineTableStatement {
    ...
    pub(crate) async fn compute(
        ...
        let table_id = if let Some(tb) = txn.get_tb(ns.namespace_id, db.database_id, &name).await? {
            ...
            txn.get_next_tb_id(Some(ctx), ns.namespace_id, db.database_id)
                .await?
        };
        ...
        let fields = txn
            .all_tb_fields(ns.namespace_id, db.database_id, &name, opt.version)
            .await?;
        ...
        if let Some(view) = &tb_def.view {
            ...
            let (ViewDefinition::Materialized { tables, .. }
            | ViewDefinition::Aggregated { tables, .. }
            | ViewDefinition::Select { tables, .. }) = &view;
            ...
            for ft in tables.iter() {
                ...
                let Some(foreign_tb) = txn.get_tb(ns.namespace_id, db.database_id, ft).await?
                    ...
                    bail!(Error::TbNotFound { name: ft.clone() });
                };
    ...
    async fn initialize_aggregate_view(
        ...
        for aggregation in analysis.aggregations.iter() {
            match aggregation {
                Aggregation::Count => {
                    ...
                    required_values
                        .entry(SelectAggr::Base(Aggregation::Count))
                        .or_insert(len);
                }
                ...
                Aggregation::Sum(arg) => {
                    let len = required_values.len();
                    required_values
                        .entry(SelectAggr::Base(Aggregation::Sum(*arg)))
                        .or_insert(len);
                }
        ...
        for r in res {
            ...
            for a in analysis.aggregations.iter() {
                match *a {
                    Aggregation::Count => {
                        ...
                        stats.push(AggregationStat::Count { count: *i });
                    }
                    Aggregation::CountValue(arg) => {
                        ...
                        stats.push(AggregationStat::CountValue { arg, count: *i });
                    }
                    Aggregation::NumberMax(arg) => {
                        ...
                        stats.push(AggregationStat::NumberMax { arg, max: *n });
                    }
                    Aggregation::NumberMin(arg) => {
                        ...
                        stats.push(AggregationStat::NumberMin { arg, min: *n });
                    }
                    Aggregation::Sum(arg) => {
                        ...
                        stats.push(AggregationStat::Sum { arg, sum: *n });
                    }
                    Aggregation::Mean(arg) => {
                        let idx = required_values[&SelectAggr::Base(Aggregation::Sum(arg))];
                        let Value::Number(n) = &aggregate_stats[idx] else {
            ...
            tx.put_record(ns, db, view_table_name, &key, record.clone(), None)
                .await?;
    ...
    pub async fn add_in_out_fields(
        ...
        if let TableType::Relation(rel) = &tb.table_type {
            // Set the `in` field as a DEFINE FIELD definition
            {
                ...
                let val = Some(Kind::Record(
                    rel.from.iter().cloned().map(TableName::new).collect(),
                ));
                txn.set(
                    &key,
                    &FieldDefinition {
            ...
            // Set the `out` field as a DEFINE FIELD definition
            {
                ...
                let val = Some(Kind::Record(
                    rel.to.iter().cloned().map(TableName::new).collect(),
                ));
                txn.set(
                    &key,
                    &FieldDefinition {
