    ...
    async fn compute(
        ...
        match self {
            Subject::Record(record_id_lit) => Ok(catalog::Subject::Record(
                record_id_lit.compute(stk, ctx, opt, doc).await?,
            )),
...
pub async fn create_grant(
    ...
    let ac = match base {
        ...
        Base::Ns => {
            ...
            txn.get_ns_access(ns, &access)
                .await?
                .ok_or_else(|| Error::AccessNsNotFound {
                    ac: access.clone(),
                    // The namespace is expected above
                    ns: opt
                        .ns
                        .as_deref()
                        .expect("namespace validated by expect_ns_id")
                        .to_owned(),
                })?
        }
        Base::Db => {
            let (ns, db) = ctx.expect_ns_db_ids(opt).await?;
            txn.get_db_access(ns, db, &access)
                .await?
                .ok_or_else(|| Error::AccessDbNotFound {
                    ac: access.clone(),
    ...
    match &ac.access_type {
        catalog::AccessType::Jwt(_) => Err(anyhow::Error::new(Error::Unimplemented(format!(
            "Grants for JWT on {base}"
        )))),
        catalog::AccessType::Record(at) => {
            ...
            let expiration = ac
                .grant_duration
                .map(|d| val::Duration(d) + Datetime::now());
...
async fn compute_show(
    ...
    match &stmt.gr {
        Some(gr) => {
            ...
            Ok(Value::Object(access_object_from_grant(
                &(*grant).clone().redacted(),
            )))
        }
        None => {
            // Get all grants.
...
async fn compute_purge(
    ...
    for gr in grs.iter() {
        ...
        if purge_expired || purge_revoked {
            match base {
                Base::Root => {
                    txn.del(&crate::key::root::access::gr::new(&stmt.ac, &gr.id))
                        .await?
                }
                Base::Ns => {
                    let ns = ctx.get_ns_id(opt).await?;
            ...
            purged.push(Value::Object(access_object_from_grant(
                &gr.clone().redacted(),
            )));
        }
    }
...
impl AccessStatement {
    ...
    pub(crate) async fn compute(
        ...
        match self {
            ...
            AccessStatement::Show(stmt) => compute_show(stmt, stk, ctx, opt, doc)
                .await
                .map_err(ControlFlow::Err),
            AccessStatement::Revoke(stmt) => compute_revoke(stmt, stk, ctx, opt, doc)
                .await
                .map_err(ControlFlow::Err),
            AccessStatement::Purge(stmt) => compute_purge(stmt, ctx, opt, doc)
                .await
                .map_err(ControlFlow::Err),
        }
    }
