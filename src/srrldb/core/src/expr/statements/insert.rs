...
impl InsertStatement {
    ...
    pub(crate) async fn compute(
        ...
        let tb = match &self.into {
            Some(into) => {
                match stk
                    .run(|stk| into.compute(stk, ctx, opt, doc))
                    .await
                    .catch_return()?
                {
        ...
        match &self.data {
            ...
            Data::SingleExpression(v) => {
                ...
                match v {
                    ...
                    v => {
                        bail!(Error::InsertStatement { value: v.to_sql() })
                    }
                }
            }
        ...
        CursorDoc::update_parent(ctx, doc, async |ctx| {
            ...
            let res = iterator
                .output(stk, &ctx, opt, &stm, RecordStrategy::KeysAndValues)
                .await?;
...
fn iterable(
    ...
    if relation {
        let f = match v.pick(&*IN) {
            Value::RecordId(v) => v,
            v => {
                bail!(Error::InsertStatementIn { value: v.to_sql() })
            }
        };
        let w = match v.pick(&*OUT) {
            Value::RecordId(v) => v,
            v => {
                bail!(Error::InsertStatementOut { value: v.to_sql() })
            }
        };
        ...
        Ok(Iterable::Relatable(
            doc_ctx,
            f,
            RelateThrough::from((tb, id)),
            w,
            Some(v),
        ))
    } else {
...
fn extract_table_and_rid_key(
    ...
    let rid = match record.rid() {
        ...
        v => {
            bail!(Error::InsertStatementId { value: v.to_sql() });
        }
    };
