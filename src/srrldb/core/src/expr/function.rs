...
impl Function {
    ...
    pub fn read_only(&self) -> bool {
        match self {
            Self::Custom(_) | Self::Script(_) | Self::Module(_, _) | Self::Silo { .. } => false,
    ...
    pub(crate) async fn compute(
        ...
        match self {
            ...
            Function::Custom(s) => {
                ...
                let result = stk
                    .run(|stk| val.block.compute(stk, &ctx, &opt, doc))
                    .await
                    .catch_return()?;
            ...
            Function::Module(module, sub) => {
                ...
                let result = executable
                    .run(stk, ctx, opt, doc, args, sub.as_deref())
                    .await?;
...
async fn check_perms(
    ...
    match permissions {
        Permission::Full => Ok(()),
        Permission::None => Err(ControlFlow::from(anyhow::Error::new(
        ...
        Permission::Specific(e) => {
            ...
            if !stk
                ...
                .is_truthy()
            {
                Err(ControlFlow::from(anyhow::Error::new(
                    Error::FunctionPermissions {
                        name: name.to_string(),
                    },
                )))
