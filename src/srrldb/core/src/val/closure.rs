...
impl Closure {
    pub(crate) async fn invoke(
        ...
        match self {
            Closure::Expr {
                ...
                let ctx = ctx.freeze();
                let result = stk
                    .run(|stk| body.compute(stk, &ctx, opt, doc))
                    .await
                    .catch_return()?;
    ...
    pub(crate) fn into_expr(self) -> Expr {
        match self {
            ...
            Closure::Builtin { .. } => Expr::Closure(Box::new(ClosureExpr {
                args: vec![],
                returns: None,
...
impl SerializeRevisioned for Closure {
    fn serialize_revisioned<W: std::io::Write>(
        ...
        Err(revision::Error::Conversion(
            "Closures cannot be stored on disk".to_string(),
        ))
    }
}
...
impl DeserializeRevisioned for Closure {
    fn deserialize_revisioned<R: std::io::Read>(_reader: &mut R) -> Result<Self, revision::Error> {
        Err(revision::Error::Conversion(
            "Closures cannot be deserialized from disk".to_string(),
        ))
    }
}
