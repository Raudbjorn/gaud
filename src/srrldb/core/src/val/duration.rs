    ...
    fn try_add(self, other: Datetime) -> Result<Datetime> {
        match chrono::Duration::from_std(self.0) {
            Ok(d) => match other.0.checked_add_signed(d) {
                ...
                None => Err(anyhow::Error::new(Error::ArithmeticOverflow(format!(
                    "{self} + {other}"
                )))),
            },
            Err(_) => Err(anyhow::Error::new(Error::ArithmeticOverflow(format!(
                "{self} + {other}"
            )))),
        }
    }
...
impl TrySub<Datetime> for Duration {
    ...
    fn try_sub(self, other: Datetime) -> Result<Datetime> {
        match chrono::Duration::from_std(self.0) {
            Ok(d) => match other.0.checked_sub_signed(d) {
                Some(v) => Ok(Datetime::from(v)),
                None => Err(anyhow::Error::new(Error::ArithmeticNegativeOverflow(
                    format!("{self} - {other}"),
                ))),
            },
            Err(_) => Err(anyhow::Error::new(Error::ArithmeticNegativeOverflow(
                format!("{self} - {other}"),
            ))),
        }
    }
