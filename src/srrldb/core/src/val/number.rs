...
impl Number {
    ...
    pub(crate) fn from_decimal_buf(b: &[u8]) -> Result<Self> {
        ...
        if dec.is_finite() {
            ...
            bail!(Error::Serialization(format!(
                "Invalid decimal value: {dec}"
            )))
        }
    }
...
impl TryNeg for Number {
    ...
    fn try_neg(self) -> Result<Self::Output> {
        Ok(match self {
            Self::Int(n) => Number::Int(
                n.checked_neg()
                    .ok_or_else(|| Error::TryNeg(n.to_string()))?,
            ),
...
mod tests {
    ...
    fn test_try_float_div() {
        ...
        assert_eq!(
            sum_one.try_float_div(count_one).unwrap(),
            Number::Float(2.5)
        );
        // i64::MIN
        ...
        assert_eq!(
            sum_three.try_float_div(count_three).unwrap(),
            Number::Float(2.1)
        );
    }
    ...
    fn serialised_ord_test() {
        ...
        for window in ordering.windows(2) {
            ...
            assert!(
                b1 < b2,
                "{n1:?} < {n2:?} (after serialization) - {b1:?} < {b2:?}"
            );
    ...
    fn serialised_test() {
        ...
        check(&[
            Number::Int(1),
            Number::Float(1.0),
            Number::Decimal(Decimal::ONE),
        ]);
        check(&[
            Number::Int(-1),
            Number::Float(-1.0),
            Number::Decimal(Decimal::NEGATIVE_ONE),
        ]);
        check(&[
            Number::Float(1.5),
            Number::Decimal(Decimal::from_str_normalized("1.5").unwrap()),
        ]);
    }
}
