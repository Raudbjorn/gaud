                ...
                Part::Lookup(g) => match this {
                    ...
                    Value::Array(x) => {
                        ...
                        stk.scope(|scope| {
                            let futs = x
                                .iter_mut()
                                .map(|v| scope.run(|stk| v.fetch(stk, ctx, opt, prev)));
                            try_join_all(futs)
                        })
                        .await?;
                        return Ok(());
                ...
                Part::Destructure(p) => match this {
                    ...
                    Value::Object(_) => {
                        for p in p.iter() {
                            ...
                            stk.run(|stk| this.fetch(stk, ctx, opt, &destructure_path))
                                .await?;
                        }
                        return Ok(());
                    }
                    ...
                    Value::Array(x) => {
                        for v in x.iter_mut() {
                            ...
                            if stk
                                ...
                                .is_truthy()
                            {
                                stk.run(|stk| v.fetch(stk, ctx, opt, iter.as_slice()))
                                    .await?;
                            }
                        }
                    }
        ...
        match this {
            Value::Array(v) => {
                stk.scope(|scope| {
                    let futs = v
                        .iter_mut()
                        .map(|v| scope.run(|stk| v.fetch(stk, ctx, opt, path)));
                    try_join_all(futs)
                })
                .await?;
                Ok(())
