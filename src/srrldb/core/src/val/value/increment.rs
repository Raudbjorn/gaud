    ...
    pub(crate) async fn increment(
        ...
        match self.get(stk, ctx, opt, None, path).await.catch_return()? {
            ...
            Value::Array(v) => match val {
                Value::Array(x) => {
                    self.set(stk, ctx, opt, path, Value::Array(v.concat(x)))
                        .await
                }
                x => {
                    self.set(stk, ctx, opt, path, Value::Array(v.with_push(x)))
                        .await
                }
            },
            Value::None => match val {
                Value::Number(x) => {
                    self.set(stk, ctx, opt, path, Value::Number(Number::Int(0) + x))
                        .await
                }
