    ...
    pub(crate) async fn get(
        ...
        match first {
            ...
            Part::Recurse(recurse, inner_path, instruction) => {
                ...
                if !after.is_empty() {
                    stk.run(|stk| v.get(stk, ctx, opt, doc, after.as_slice()))
                        .await
                } else {
                    Ok(v)
                }
            ...
            Part::RepeatRecurse => Err(ControlFlow::Err(anyhow::Error::new(
            ...
            p => match self {
                ...
                Value::Geometry(v) => match p {
                    ...
                    Part::Optional => {
                        stk.run(|stk| self.get(stk, ctx, opt, doc, path.next()))
                            .await
                    }
                ...
                Value::Object(v) => match p {
                    ...
                    Part::Destructure(p) => {
                        ...
                        for p in p.iter() {
                            ...
                            obj.insert(
                                p.field().to_owned(),
                                stk.run(|stk| idiom.compute(stk, ctx, opt, Some(&cur_doc)))
                                    .await?,
                            );
                        }

                        let obj = Value::from(obj);
                        stk.run(|stk| obj.get(stk, ctx, opt, doc, path.next()))
                            .await
                    }
                    Part::Method(name, args) => {
                        let args = stk
                        ...
                        stk.run(|stk| res.get(stk, ctx, opt, doc, path.next()))
                            .await
                    }
                    Part::Optional => {
                        stk.run(|stk| self.get(stk, ctx, opt, doc, path.next()))
...
mod tests {
    ...
    async fn get_basic_deep_ok() {
        let (ctx, opt) = mock().await;
        let depth = 20;
        let idi: Idiom = syn::idiom(&format!("{}something", "test.".repeat(depth)))
            .unwrap()
            .into();
    ...
    async fn get_array_fields() {
        ...
        assert_eq!(
            res,
            [Value::from(34i64), Value::from(36i64)]
                .into_iter()
                .collect::<Value>()
        );
    }
    ...
    async fn get_array_where_field() {
        let (ctx, opt) = mock().await;
        let idi: Idiom = syn::idiom("test.something[WHERE age > 35].age")
            .unwrap()
            .into();
        let val: Value = parse_val!("{ test: { something: [{ age: 34 }, { age: 36 }] } }");
        let mut stack = reblessive::tree::TreeStack::new();
    ...
    async fn get_array_where_fields_array_index() {
        let (ctx, opt) = mock().await;
        let idi: Idiom = syn::idiom("test.something[WHERE age > 30][0]")
            .unwrap()
            .into();
        let val: Value = parse_val!("{ test: { something: [{ age: 34 }, { age: 36 }] } }");
        let mut stack = reblessive::tree::TreeStack::new();
