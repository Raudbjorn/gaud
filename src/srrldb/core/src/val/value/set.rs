    ...
    pub(crate) async fn set(
        ...
        while let Some(p) = iter.next() {
            ...
            match p {
                ...
                Part::Value(x) => {
                    let v = stk
                        .run(|stk| x.compute(stk, ctx, opt, None))
                        .await
                        .catch_return()?;
                ...
                Part::Where(w) => {
                        ...
                        .is_some()
                    {
                        ...
                        stk.run(|stk| a.set(stk, ctx, opt, iter.as_slice(), val.clone()))
                            .await?;
...
mod tests {
    ...
    async fn set_none() {
        ...
        stack
            .enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(999)))
            .finish()
            .await
            .unwrap();
        assert_eq!(res, val);
    ...
    async fn set_array_field() {
        ...
        let mut stack = reblessive::TreeStack::new();
        stack
            .enter(|stk| val.set(stk, &ctx, &opt, &idi, Value::from(21)))
            .finish()
            .await
            .unwrap();
        assert_eq!(res, val);
    ...
    async fn set_array_where_fields_array_index_field() {
        let (ctx, opt) = mock().await;
        let idi: Idiom = syn::idiom("test.something[WHERE age > 30][0].age")
            .unwrap()
            .into();
        let mut val = parse_val!("{ test: { something: [{ age: 34 }, { age: 36 }] } }");
    ...
    async fn set_object_with_new_nested_array_access_field_in_array_with_thing() {
        let (ctx, opt) = mock().await;
        let idi: Idiom = syn::idiom("test.something.other[city:london]")
            .unwrap()
            .into();
        let mut val = parse_val!("{ test: { something: [{ age: 34 }, { age: 36 }] } }");
        let res = parse_val!(
