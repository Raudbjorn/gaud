...
impl fmt::Display for CastError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CastError::InvalidKind { from, into } => {
                write!(
                    f,
                    "Could not cast into `{into}` using input `{from}`",
                    from = from.to_sql()
                )
            }
            CastError::ElementOf { inner, into } => {
                inner.fmt(f)?;
                write!(f, " when coercing an element of `{into}`")
            }
            CastError::InvalidLength { len, into } => {
                write!(
                    f,
                    "Expected `{into}` but found a collection of length `{len}`"
                )
            }
            CastError::RangeSizeLimit { value } => {
                write!(
                    f,
            ...
            Value::Array(x) => {
                ...
                for v in x.0 {
                    ...
                    let x = v
                        .clone()
                        .cast_to::<i64>()
                        .expect("value checked to be castable above");
                    // TODO: Fix truncation.
                    res.push(x as u8);
                }
    ...
    fn cast(v: Value) -> Result<Self, CastError> {
        match v {
            ...
            Value::Range(range) => {
                ...
                let range = range
                    .coerce_to_typed::<i64>()
                    .expect("range type checked above");
            ...
            Value::Bytes(x) => Ok(Array(
                x.0.into_iter().map(|x| Value::from(x as i64)).collect(),
            )),
            _ => Err(CastError::InvalidKind {
                from: v,
                into: "array".into(),
    ...
    pub fn cast_to_kind(self, kind: &Kind) -> Result<Value, CastError> {
        ...
        match kind {
            ...
            Kind::Table(t) => match t.is_empty() {
                true => self
                    .cast_to::<String>()
                    .map(|s| Value::Table(crate::val::TableName::new(s))),
