...
impl fmt::Display for CoerceError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            CoerceError::InvalidKind { from, into } => {
                write!(
                    f,
                    "Expected `{into}` but found `{from}`",
                    from = from.to_sql()
                )
            }
            CoerceError::ElementOf { inner, into } => {
                inner.fmt(f)?;
                write!(f, " when coercing an element of `{into}`")
            }
            CoerceError::InvalidLength { len, into } => {
                write!(
                    f,
...
impl<T: Coerce + HasKind> Coerce for Vec<T> {
    ...
    fn coerce(v: Value) -> Result<Self, CoerceError> {
        ...
        for x in array.0 {
            ...
            res.push(
                x.coerce_to::<T>()
                    .with_element_of(|| <Self as HasKind>::kind().to_sql())?,
            )
        }
        Ok(res)
    }
