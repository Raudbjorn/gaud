    ...
    pub(crate) async fn decrement(
        ...
        match self.get(stk, ctx, opt, None, path).await.catch_return()? {
            ...
            Value::Array(v) => match val {
                Value::Array(x) => {
                    self.set(stk, ctx, opt, path, Value::from(v.remove_all(&x.0)))
                        .await
                }
                x => {
                    self.set(stk, ctx, opt, path, Value::from(v.remove_value(&x)))
                        .await
                }
            },
            Value::None => match val {
                Value::Number(x) => {
                    self.set(stk, ctx, opt, path, Value::from(Number::from(0) - x))
                        .await
                }
                _ => Ok(()),
            },
