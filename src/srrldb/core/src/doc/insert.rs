...
impl Document {
    pub(super) async fn insert(
        ...
        if !self.is_iteration_initial() {
            return self
                .insert_update(stk, ctx, opt, &Statement::Insert(stm))
                .await;
        }
        ...
        let retry = match self
            .insert_create(stk, ctx, opt, &Statement::Insert(stm))
            .await
        {
            ...
            Err(IgnoreError::Error(e)) => match e.downcast_ref::<err::Error>() {
                Some(Error::IndexExists { .. }) => {
                    ...
                    let Ok(Error::IndexExists { record: thing, .. }) = e.downcast() else {
                        // Checked above
                        unreachable!()
                    };
                    thing
                ...
                Some(Error::RecordExists { .. }) => {
                    ...
                    let Ok(Error::RecordExists { record: thing, .. }) = e.downcast() else {
                        // Checked above
                        unreachable!()
                    };
                    thing
        ...
        let val = ctx
            .tx()
            .get_record(ns, db, &retry.table, &retry.key, opt.version)
            .await?;
        ...
        self.insert_update(stk, ctx, opt, &Statement::Insert(stm))
            .await
    }
