...
impl Document {
        ...
        match view {
            ...
            ViewDefinition::Materialized {
                ...
                if set {
                    ...
                    ctx.tx()
                        .del_record(db.namespace_id, db.database_id, table_name, id)
                        .await?;
                }
                Ok(())
            }
        ...
        match action {
            Action::Create => {
                ...
                for g in aggr.group_expressions.iter() {
                    group.push(
                        g.compute(stk, ctx, opt, Some(&self.current))
                            .await
                            .catch_return()?,
                    );
                }
            ...
            Action::Update => {
                ...
                let group_before = if before_cond {
                    ...
                    for g in aggr.group_expressions.iter() {
                        group.push(
                            g.compute(stk, ctx, opt, Some(&self.initial))
                                .await
                                .catch_return()?,
                        );
                    }
    ...
    async fn process_view_record_create(
        ...
        for a in aggr.aggregate_arguments.iter() {
            args.push(
                a.compute(stk, ctx, opt, Some(&self.current))
                    .await
                    .catch_return()?,
            )
        }
        ...
        let doc = Value::Object(aggregation::create_field_document(
            &group,
            &meta.aggregation_stats,
        ))
        .into();
        ...
        let tb = ctx
            .tx()
            .get_or_add_tb(Some(ctx), &ns.name, &db.name, view_table_name)
            .await?;
        let fields = ctx
            .tx()
            .all_tb_fields(
    ...
    async fn process_view_record_delete(
        ...
        for a in aggr.aggregate_arguments.iter() {
            args.push(
                a.compute(stk, ctx, opt, Some(&self.initial))
                    .await
                    .catch_return()?,
            )
        }
        ...
        if !recalculations.is_empty() {
            ...
            for (idx, v) in values.into_iter().enumerate() {
                match &mut meta.aggregation_stats[recalculations[idx].stat] {
                    AggregationStat::TimeMin { min: stat, .. }
                    | AggregationStat::TimeMax { max: stat, .. } => {
                    ...
                    AggregationStat::NumberMin { min: stat, .. }
                    | AggregationStat::NumberMax { max: stat, .. } => {
    ...
    pub(crate) async fn run_triggers(
        ...
        stk.run(|stk| document.store_index_data(stk, ctx, opt))
            .await?;
        stk.run(|stk| document.process_views(stk, ctx, opt, action))
            .await?;
        stk.run(|stk| document.process_events(stk, ctx, opt, action, None))
            .await?;
