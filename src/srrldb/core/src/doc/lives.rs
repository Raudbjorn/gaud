...
impl Document {
    ...
    async fn lq_compute(
        ...
        let mut doc = match (self.check_reduction_required(&opt)?, is_delete) {
            (true, true) => {
                self.compute_reduced_target(stk, &ctx, &opt, &self.initial)
                    .await?
            }
            (true, false) => {
                self.compute_reduced_target(stk, &ctx, &opt, &self.current)
                    .await?
            }
        ...
        match self
            .lq_check(stk, &ctx, &opt, &live_subscription, &doc)
            .await
        {
            ...
            SubscriptionFields::Diff => {
                ...
                if is_delete {
                    ...
                    let result = Value::Array(
                        operations
                            .into_iter()
                            .map(|op| Value::Object(op.into_object()))
                            .collect(),
                    );
