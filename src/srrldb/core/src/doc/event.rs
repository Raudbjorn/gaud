...
impl Document {
    ...
    pub(super) async fn process_events(
        ...
        for ev in self.ev(ctx, opt).await?.iter() {
            ...
            if val.is_truthy() {
                if let EventKind::Async { .. } = ev.kind {
    ...
    async fn build_event_options(
        ...
        if ns.namespace_id != eq.ns {
            bail!(Error::EvNamespaceMismatch(
                self.event_definition.name.clone(),
                ns.name.clone()
            ));
        }
...
impl AsyncEventContext {
    ...
    async fn run_event_checked(mut self, stk: &mut Stk) {
        ...
        {
            error!(
                "Unexpected error while processing an event. Error: {e} - Key: {:?}",
                self.k
            );
        }
    }
    ...
    async fn new_write_tx(&self) -> Result<Transaction> {
        self.tf
            .transaction(Write, LockType::Optimistic, self.sequences.clone())
            .await
    }
