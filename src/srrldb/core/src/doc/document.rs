...
impl Document {
    ...
    pub(crate) fn is_condition_checked(&self) -> bool {
        matches!(
            ...
            RecordStrategy::Count | RecordStrategy::KeysOnly
        )
    }
    ...
    pub(crate) async fn reduced(
        ...
        match permitted {
            Permitted::Initial => {
                self.initial_reduced = self
                    .compute_reduced_target(stk, ctx, opt, &self.initial)
                    .await?;
            }
            Permitted::Current => {
                self.current_reduced = self
                    .compute_reduced_target(stk, ctx, opt, &self.current)
                    .await?;
            }
            Permitted::Both => {
                self.initial_reduced = self
