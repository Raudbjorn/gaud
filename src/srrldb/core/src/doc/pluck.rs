            ...
            Some(v) => match v {
                ...
                Output::Diff => {
                    ...
                    let (initial, current) = if self.reduced(stk, ctx, opt, Both).await? {
                        ...
                        self.computed_fields(stk, ctx, opt, DocKind::InitialReduced)
                            .await?;
                        self.computed_fields(stk, ctx, opt, DocKind::CurrentReduced)
                            .await?;
                        (&mut self.initial_reduced, &mut self.current_reduced)
                    } else {
                        // Compute the computed fields
                        self.computed_fields(stk, ctx, opt, DocKind::Initial)
                            .await?;
                        self.computed_fields(stk, ctx, opt, DocKind::Current)
                            .await?;
                ...
                Output::Fields(v) => {
                    ...
                    v.compute(stk, &ctx, opt, Some(current))
                        .await
