...
pub(crate) fn convert_public_value_to_internal(value: srrldb_types::Value) -> crate::val::Value {
    match value {
        ...
        srrldb_types::Value::Number(n) => match n {
            srrldb_types::Number::Int(i) => crate::val::Value::Number(crate::val::Number::Int(i)),
        ...
        srrldb_types::Value::Uuid(u) => crate::val::Value::Uuid(crate::val::Uuid(u.into_inner())),
        ...
        srrldb_types::Value::RecordId(PublicRecordId { table, key }) => {
...
fn convert_public_record_id_key_to_internal(
    ...
    match key {
        ...
        srrldb_types::RecordIdKey::Array(a) => crate::val::RecordIdKey::Array(crate::val::Array(
            a.into_iter()
                .map(convert_public_value_to_internal)
                .collect(),
        )),
...
impl From<Expr> for crate::expr::Expr {
    fn from(v: Expr) -> Self {
        match v {
            ...
            Expr::Prefix { op, expr } => crate::expr::Expr::Prefix {
                op: op.into(),
                expr: Box::new((*expr).into()),
            },
            Expr::Postfix { op, expr } => crate::expr::Expr::Postfix {
                op: op.into(),
                expr: Box::new((*expr).into()),
            },

            Expr::Binary { left, op, right } => crate::expr::Expr::Binary {
...
impl From<crate::expr::Expr> for Expr {
    fn from(v: crate::expr::Expr) -> Self {
        match v {
            ...
            crate::expr::Expr::Prefix { op, expr } => Expr::Prefix {
                op: op.into(),
                expr: Box::new((*expr).into()),
            },
            crate::expr::Expr::Postfix { expr, op } => Expr::Postfix {
                expr: Box::new((*expr).into()),
                op: op.into(),
            },

            crate::expr::Expr::Binary { left, op, right } => Expr::Binary {
                left: Box::new((*left).into()),
                op: op.into(),
                right: Box::new((*right).into()),
            },
