    ...
    Ok(match check {
        Some(Value::Closure(closure)) => {
            if let Some(opt) = opt {
                for arg in array {
                    if closure
                        .invoke(stk, ctx, opt, doc, vec![arg])
                        .await?
                        .is_truthy()
                    {
                        continue;
    ...
    Ok(match check {
        Value::Closure(closure) => {
            if let Some(opt) = opt {
                ...
                for arg in array {
                    if closure
                        .invoke(stk, ctx, opt, doc, vec![arg.clone()])
                        .await?
                        .is_truthy()
                    {
                        res.push(arg)
...
pub async fn fold(
    ...
    if let Some(opt) = opt {
        ...
        for (i, val) in array.into_iter().enumerate() {
            accum = mapper
                .invoke(stk, ctx, opt, doc, vec![accum, val, i.into()])
                .await?
        }
        Ok(accum)
    } else {
...
pub async fn map(
    ...
    if let Some(opt) = opt {
        ...
        for (i, arg) in array.into_iter().enumerate() {
            res.push(
                mapper
                    .invoke(stk, ctx, opt, doc, vec![arg, i.into()])
                    .await?,
            );
        }
        Ok(res.into())
        ...
        match array.len() {
            ...
            _ => {
                ...
                for (idx, val) in iter.enumerate() {
                    accum = mapper
                        .invoke(stk, ctx, opt, doc, vec![accum, val, idx.into()])
                        .await?;
                }
                Ok(accum)
            }
...
pub fn repeat((value, count): (Value, i64)) -> Result<Value> {
    ...
    limit(
        "array::repeat",
        mem::size_of::<Value>().saturating_mul(count),
    )?;
    ...
    fn array_slice() {
        ...
        fn test(initial: &[u8], beg: Option<i64>, end: Option<i64>, expected: &[u8]) {
            let initial_values = initial
                ...
                .collect::<Vec<_>>()
                .into();
            let expected_values: Array = expected
                .iter()
                .map(|n| Value::from(*n as i64))
                .collect::<Vec<_>>()
                .into();
