...
impl Aggregation {
    pub fn to_stat(&self) -> AggregationStat {
        match *self {
            Aggregation::Count => AggregationStat::Count { count: 0 },
            Aggregation::CountValue(arg) => AggregationStat::CountValue { arg, count: 0 },
...
impl AggregationStat {
    ...
    pub fn get_count(aggregation_stats: &[AggregationStat]) -> Option<i64> {
        aggregation_stats.iter().find_map(|x| match x {
            AggregationStat::Count { count }
            | AggregationStat::Mean { count, .. }
            | AggregationStat::Variance { count, .. }
            | AggregationStat::StdDev { count, .. } => Some(*count),
            _ => None,
        })
    }
}
...
pub fn add_to_aggregation_stats(arguments: &[Value], stats: &mut [AggregationStat]) -> Result<()> {
    for stat in stats {
        match stat {
            ...
            AggregationStat::CountValue { arg, count } => {
                *count += arguments[*arg].is_truthy() as i64;
            }
            AggregationStat::NumberMax { arg, max } => {
            ...
            AggregationStat::NumberMin { arg, min } => {
                let Value::Number(ref n) = arguments[*arg] else {
                    bail!(Error::InvalidArguments {
            ...
            AggregationStat::Mean { arg, sum, count } => {
                let Value::Number(ref n) = arguments[*arg] else {
                    bail!(Error::InvalidArguments {
            ...
            AggregationStat::TimeMax { arg, max } => {
            ...
            AggregationStat::TimeMin { arg, min } => {
                let Value::Datetime(ref d) = arguments[*arg] else {
                    bail!(Error::InvalidArguments {
            ...
            AggregationStat::Accumulate { arg, values } => {
...
pub fn create_field_document(group: &[Value], stats: &[AggregationStat]) -> Object {
    ...
    for (idx, a) in stats.iter().enumerate() {
        let value = match a {
            AggregationStat::Count { count } | AggregationStat::CountValue { count, .. } => {
                Value::from(Number::from(*count))
            }
            AggregationStat::NumberMax { max, .. } => (*max).into(),
            AggregationStat::NumberMin { min, .. } => (*min).into(),
            AggregationStat::Sum { sum, .. } => (*sum).into(),
            AggregationStat::Mean { sum, count, .. } => sum
                .try_float_div((*count).into())
            ...
            AggregationStat::TimeMax { max, .. } => max.clone().into(),
            AggregationStat::TimeMin { min, .. } => min.clone().into(),
            AggregationStat::Accumulate { values, .. } => Value::Array(Array(values.clone())),
        };
    ...
    fn visit_mut_expr(&mut self, s: &mut Expr) -> Result<(), Self::Error> {
        match s {
            Expr::FunctionCall(f) => {
                ...
                *s = Expr::Idiom(Idiom::field(aggregate_field_name(
        ...
        let fields = match fields {
            ...
            Fields::Select(fields) => {
                ...
                for f in fields.iter() {
                    let Field::Single(Selector { expr, alias }) = f else {
                        // all is not a valid aggregate selector.
                        bail!(Error::InvalidAggregationSelector { expr: f.to_sql() })
                    };
                    ...
                    if let Some((alias, x)) = alias.as_ref().and_then(|alias| {
                        ...
                        collect_fields.push((
                            alias.clone(),
                            Expr::Idiom(Idiom::field(group_field_name(x))),
                        ));
                    } else {
