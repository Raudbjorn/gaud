...
pub fn field_with_type() -> FieldDefinition {
    FieldDefinition {
        ...
        value: Some(Expr::Literal(Literal::String(
            "string::lowercase($value)".to_string(),
        ))),
        assert: Some(Expr::Literal(Literal::String(
            "string::is::email($value)".to_string(),
        ))),
        computed: None,
...
pub fn function_basic() -> FunctionDefinition {
    FunctionDefinition {
        ...
        args: vec![],
        block: Block(vec![Expr::Literal(Literal::String(
...
pub fn record_number_decimal() -> Record {
    Record::new(Data::from(Value::Number(Number::Decimal(Decimal::from(
        42,
    )))))
}
...
pub fn record_bytes() -> Record {
    Record::new(Data::from(Value::Bytes(Bytes::from(vec![
        0x01, 0x02, 0x03, 0x04,
    ]))))
}
...
pub fn record_geometry_point() -> Record {
    Record::new(Data::from(Value::Geometry(Geometry::Point(Point::new(
        1.0, 2.0,
    )))))
}
...
pub fn record_geometry_line() -> Record {
    Record::new(Data::from(Value::Geometry(Geometry::Line(
...
pub fn record_geometry_polygon() -> Record {
    Record::new(Data::from(Value::Geometry(Geometry::Polygon(
        Polygon::new(
            LineString::new(vec![
                coord! { x: 1.0, y: 2.0 },
                coord! { x: 3.0, y: 4.0 },
...
pub fn record_geometry_multi_point() -> Record {
    Record::new(Data::from(Value::Geometry(Geometry::MultiPoint(
        MultiPoint::new(vec![Point::new(1.0, 2.0), Point::new(3.0, 4.0)]),
    ))))
}

/// Record with geometry data (multi line)
pub fn record_geometry_multi_line() -> Record {
    Record::new(Data::from(Value::Geometry(Geometry::MultiLine(
...
pub fn record_geometry_multi_polygon() -> Record {
    Record::new(Data::from(Value::Geometry(Geometry::MultiPolygon(
...
pub fn record_recordid() -> Record {
    Record::new(Data::from(Value::RecordId(RecordId::new(
...
pub fn record_file() -> Record {
    Record::new(Data::from(Value::File(File::new(
        "bucket".to_string(),
        "key".to_string(),
    ))))
}
...
pub fn record_regex() -> Record {
    Record::new(Data::from(Value::Regex(
        Regex::from_str("^test.*").unwrap(),
    )))
}
...
pub fn table_mutations_set() -> TableMutations {
    ...
    let mut obj = Object::default();
    obj.insert("name".to_string(), Value::String("Alice".to_string()));
    mutations.1.push(TableMutation::Set(
...
pub fn table_mutations_del() -> TableMutations {
    let mut mutations = TableMutations::new(TableName::from("users"));
    mutations.1.push(TableMutation::Del(RecordId::new(
        TableName::from("users"),
        1,
    )));
    mutations
...
pub fn node_active() -> Node {
    Node::new(UuidExt::nil(), Timestamp { value: 1234567890 }, false)
}

/// Archived node
pub fn node_archived() -> Node {
    Node::new(UuidExt::nil(), Timestamp { value: 9876543210 }, true)
}
