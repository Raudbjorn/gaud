...
pub async fn token(kvs: &Datastore, session: &mut Session, token: &str) -> Result<()> {
    ...
    match &token_data.claims {
        ...
        Claims {
            ...
            let db_def = match catch!(tx, tx.get_db_by_name(ns, db).await) {
                ...
                None => {
                    ...
                    return Err(Error::DbNotFound { name: db.clone() }.into());
                }
            };
            // Parse the record id
            ...
            let Some(de) = catch!(
                tx,
                tx.get_db_access(db_def.namespace_id, db_def.database_id, ac)
                    .await
            ) else {
                let _ = tx.cancel().await;
        ...
        Claims {
            ...
            // Log the decoded authentication claims
            trace!(
                "Authenticating to database `{}` with access method `{}`",
                db, ac
            );
            // Create a new readonly transaction
            let tx = kvs.transaction(Read, Optimistic).await?;
            ...
            match &de.access_type {
                ...
                | catalog::AccessType::Bearer(catalog::BearerAccess { jwt, .. }) => {
                    ...
                    // Log the success
                    debug!(
                        "Authenticated to database `{}` with access method `{}`",
                        db, ac
                    );
                    // Set the session
                    session.tk = Some(
                ...
                catalog::AccessType::Record(at) => match &de.authenticate {
                    Some(au) => {
                        trace!(
                            "Access method `{}` is record access with AUTHENTICATE clause",
                            ac
                        );
        ...
        Claims {
            ...
            let de = match catch!(
                tx,
                tx.get_db_user(db_def.namespace_id, db_def.database_id, id)
                    .await
                    .map_err(|e| {
            ...
            // Log the success
            debug!(
                "Authenticated to database `{}` with user `{}` using token",
                db, id
            );
            // Set the session
            session.tk = Some(
        ...
        Claims {
            ...
            // Log the decoded authentication claims
            trace!(
                "Authenticating to namespace `{}` with access method `{}`",
                ns, ac
            );
            // Create a new readonly transaction
            let tx = kvs.transaction(Read, Optimistic).await?;
            let ns_def = match catch!(tx, tx.get_ns_by_name(ns).await) {
                ...
                None => {
                    let _ = tx.cancel().await;
                    return Err(Error::NsNotFound { name: ns.clone() }.into());
                }
            };
            ...
            let cf = match &de.access_type {
                catalog::AccessType::Jwt(jwt)
                | catalog::AccessType::Bearer(catalog::BearerAccess { jwt, .. }) => match &jwt
                    .verify
                {
            ...
            // Log the success
            debug!(
                "Authenticated to namespace `{}` with access method `{}`",
                ns, ac
            );
            // Set the session
            session.tk = Some(
        ...
        Claims {
            ...
            // Log the success
            debug!(
                "Authenticated to namespace `{}` with user `{}` using token",
                ns, id
            );
            // Set the session
            session.tk = Some(
        ...
        Claims { ac: Some(ac), .. } => {
            ...
            let Some(de) = de else {
                return Err(Error::AccessRootNotFound { ac: ac.clone() }.into());
            };
...
pub async fn verify_ns_creds(
    ...
    let user = catch!(
        tx,
        tx.get_ns_user(ns_def.namespace_id, user)
            .await
            .map_err(|e| {
...
pub async fn verify_db_creds(
    ...
    let user = catch!(
        tx,
        tx.get_db_user(db_def.namespace_id, db_def.database_id, user)
            .await
            .map_err(|e| {
...
mod tests {
    ...
    async fn test_token(
        ...
        claims.roles = roles
            .clone()
            .map(|roles| roles.into_iter().map(String::from).collect());
    ...
    async fn test_token_record_custom_claims() {
        ...
        let later = (Utc::now() + Duration::hours(1)).timestamp();
        {
            ...
            assert_eq!(
                *string_claim,
                crate::types::PublicValue::String("test".into())
            );
            ...
            assert_eq!(
                *float_claim,
                crate::types::PublicValue::Number(123.456.into())
            );
            ...
            test_object.insert(
                "test_1".to_string(),
                crate::types::PublicValue::String("value_1".into()),
            );
            ...
            assert_eq!(
                *object_claim,
                crate::types::PublicValue::Object(test_object.into())
            );
        }
    }
    ...
    fn test_verify_pass() {
        ...
        let hash = Argon2::default()
            .hash_password("test".as_bytes(), &salt)
            .unwrap()
            .to_string();
    ...
    async fn test_verify_creds_invalid() {
        ...
        {
            assert!(
                verify_db_creds(&ds, &ns, &db, "test", "test")
                    .await
                    .is_err()
            );
        }
        ...
        for level in &test_levels {
            for case in &test_cases {
                ...
                if let Some(expected_err) = &case.expected_error {
                    ...
                    assert!(
                        res.is_ok(),
                        "Failed to sign in with token for case: {:?}",
                        case
                    );
                    assert_eq!(sess.ns, level.ns.map(|s| s.to_string()));
                    assert_eq!(sess.db, level.db.map(|s| s.to_string()));
