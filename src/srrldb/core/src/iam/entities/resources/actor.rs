...
impl Actor {
    ...
    pub(crate) fn has_editor_role(&self) -> bool {
        self.roles
            .iter()
            .any(|r| r.eq(&Role::Owner) || r.eq(&Role::Editor))
    }
    ...
    pub(crate) fn has_viewer_role(&self) -> bool {
        self.roles
            .iter()
            .any(|r| r.eq(&Role::Owner) || r.eq(&Role::Editor) || r.eq(&Role::Viewer))
    }
    ...
    pub(crate) fn max_role(&self) -> Option<Role> {
        ...
        Some(self.roles.iter().fold(
            Role::Viewer,
            |max_role, &role| {
                if role > max_role { role } else { max_role }
            },
        ))
    }
}
