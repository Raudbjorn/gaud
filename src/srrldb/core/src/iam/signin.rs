...
pub async fn signin_bearer(
    ...
    let gr = match (&ns, &db) {
        (Some(ns), Some(db)) => {
                ...
                tx,
                tx.get_db_access_grant(ns.namespace_id, db.database_id, &av.name, &kid)
                    .await
            )
        }
        (Some(ns), None) => {
                ...
                tx,
                tx.get_ns_access_grant(ns.namespace_id, &av.name, &kid)
                    .await
            )
        }
    ...
    if let Some(au) = &av.authenticate {
        ...
        let mut sess = match (&ns, &db) {
            (Some(ns), Some(db)) => Session::for_level(
                Level::Database(ns.name.clone(), db.name.clone()),
                Role::Editor,
            ),
    ...
    trace!(
        "Signing in to database with bearer access method `{}`",
        av.name
    );
    // Create the authentication token.
    let enc = encode(
...
pub fn validate_grant_bearer(key: &str) -> Result<String> {
    ...
    // Check the length of the key identifier.
        ...
        kid.len() == access::GRANT_BEARER_ID_LENGTH,
    ...
    // Check the length of the key.
        ...
        key.len() == access::GRANT_BEARER_KEY_LENGTH,
        Error::AccessGrantBearerInvalid
    );
    ...
    match (&gr.expiration, &gr.revocation) {
        ...
        (Some(exp), None) => {
            if exp < &Datetime::now() {
                ...
                debug!(
                    "Bearer access grant `{}` for method `{}` is expired",
                    gr.id, gr.ac
                );
        ...
        (_, Some(_)) => {
            debug!(
                "Bearer access grant `{}` for method `{}` is revoked",
                gr.id, gr.ac
            );
            bail!(Error::InvalidAuth);
        }
    ...
    match &gr.grant {
        catalog::Grant::Bearer(bearer) => {
            ...
            if ok {
                ...
                debug!(
                    "Bearer access grant `{}` for method `{}` is invalid",
                    gr.id, gr.ac
                );
                Err(anyhow::Error::new(Error::InvalidAuth))
            }
    ...
    async fn test_signin_record_with_refresh() {
        ...
        // Test with refresh
        {
            ...
            let refresh = match res {
                Ok(data) => match data {
                    Token::WithRefresh { refresh, .. } => refresh,
...
-----END PRIVATE KEY-----"#;
            ...
            if let Ok(sd) = res {
                let token = match &sd {
                    ...
                    Token::WithRefresh { access: token, .. } => token,
                };
            ...
            // Test that only the key hash is stored
            {
                ...
                let grant = match level.level {
                    ...
                    "NS" => {
                        ...
                        tx.get_ns_access_grant(ns.namespace_id, "api", &id)
                            .await
                            .unwrap()
                    }
    ...
    async fn test_signin_bearer_for_record() {
        ...
        // Test with revoked grant
        {
            ...
            // Revoke grant
            ds.execute(
                &format!("ACCESS api ON DATABASE REVOKE GRANT {kid}"),
                &sess,
                None,
            )
            .await
        ...
        // Test with removed access method
        {
            ...
            // Remove bearer access method
            ds.execute("REMOVE ACCESS api ON DATABASE", &sess, None)
                .await
                .unwrap();
