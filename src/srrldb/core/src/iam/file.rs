            ...
            if trimmed.is_empty() {
                ...
                let path = if canonicalize {
                    let Ok(path) = fs::canonicalize(&path) else {
                        warn!(
                            "Failed to canonicalize {subject} path: {}",
...
mod tests {
    ...
    fn test_empty_allow_list_allows_access() {
        ...
        assert!(
            result.is_ok(),
            "File access should be allowed when no restrictions are set"
        );
    }
    ...
    fn test_allow_list_access() {
        ...
        assert!(
            res1.is_ok(),
            "File in the first allowed directory should be permitted"
        );
        assert!(
            res2.is_ok(),
            "File in the second allowed directory should be permitted"
        );
        ...
        assert!(
            res_outside.is_err(),
            "File outside allowed directories should be denied"
        );
    }
}
