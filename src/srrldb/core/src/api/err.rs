...
pub enum ApiError {
    ...
    InvalidRequestBodyType { expected: String, actual: String },

    #[error("Failed to parse request in middleware: {middleware}")]
    MiddlewareRequestParseFailure { middleware: String },

    #[error("Failed to resolve middleware function: {function}")]
    MiddlewareFunctionNotFound { function: String },
...
impl ApiError {
    pub fn status_code(&self) -> StatusCode {
        match self {
            ...
            Self::InvalidRequestBodyType { .. } => StatusCode::BAD_REQUEST,
            Self::MiddlewareRequestParseFailure { .. } => StatusCode::BAD_REQUEST,
            Self::MiddlewareFunctionNotFound { .. } => StatusCode::INTERNAL_SERVER_ERROR,
