...
impl Parser<'_> {
    ...
    pub(super) async fn parse_top_level_expr(
        ...
        match token.kind {
            ...
            t!("LIVE") => {
                self.pop_peek();
                self.parse_live_stmt(stk)
                    .await
                    .map(|x| TopLevelExpr::Live(Box::new(x)))
            }
            t!("OPTION") => {
                self.pop_peek();
            ...
            t!("ACCESS") => {
                self.pop_peek();
                self.parse_access(stk)
                    .await
                    .map(|x| TopLevelExpr::Access(Box::new(x)))
            }
            t!("SHOW") => {
                self.pop_peek();
    ...
    pub(super) fn parse_kill_stmt(&mut self) -> ParseResult<KillStatement> {
        ...
        let id = match peek.kind {
            t!("u\"") | t!("u'") => self
                .next_token_value()
                .map(|u| Expr::Literal(Literal::Uuid(u)))?,
    ...
    pub(super) async fn parse_let_stmt(&mut self, stk: &mut Stk) -> ParseResult<SetStatement> {
        ...
        Ok(SetStatement { name, what, kind })
    }
