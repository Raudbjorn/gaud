    ...
    pub(crate) async fn parse_remove_stmt(
        ...
        let res = match next.kind {
            ...
            t!("FUNCTION") => {
                ...
                RemoveStatement::Function(RemoveFunctionStatement { name, if_exists })
            }
            t!("MODULE") => {
                ...
                RemoveStatement::Module(RemoveModuleStatement { name, if_exists })
            }
            t!("ACCESS") => {
                let if_exists = if self.eat(t!("IF")) {
                    expected!(self, t!("EXISTS"));
            ...
            t!("ANALYZER") => {
                ...
                RemoveStatement::Analyzer(RemoveAnalyzerStatement { name, if_exists })
            }
            t!("SEQUENCE") => {
                let if_exists = if self.eat(t!("IF")) {
                    expected!(self, t!("EXISTS"));
                ...
                let name = stk.run(|stk| self.parse_expr_field(stk)).await?;
                RemoveStatement::Sequence(RemoveSequenceStatement { name, if_exists })
            }
            t!("USER") => {
                let if_exists = if self.eat(t!("IF")) {
                    expected!(self, t!("EXISTS"));
            ...
            t!("API") => {
                ...
                RemoveStatement::Api(RemoveApiStatement { name, if_exists })
            }
            t!("BUCKET") => {
                let if_exists = if self.eat(t!("IF")) {
                    expected!(self, t!("EXISTS"));
                ...
                RemoveStatement::Bucket(RemoveBucketStatement { name, if_exists })
            }
