...
impl Parser<'_> {
    pub(crate) async fn parse_define_stmt(
        ...
        match next.kind {
            t!("NAMESPACE") => self
                .parse_define_namespace(stk)
                .await
                .map(DefineStatement::Namespace),
            t!("DATABASE") => self
                .parse_define_database(stk)
                .await
                .map(DefineStatement::Database),
            t!("FUNCTION") => self
                .parse_define_function(stk)
                .await
                .map(DefineStatement::Function),
            t!("USER") => self.parse_define_user(stk).await.map(DefineStatement::User),
            t!("PARAM") => self
                .parse_define_param(stk)
                .await
                .map(DefineStatement::Param),
            t!("TABLE") => self
                .parse_define_table(stk)
                .await
                .map(DefineStatement::Table),
            t!("API") => self.parse_define_api(stk).await.map(DefineStatement::Api),
            t!("EVENT") => stk
                .run(|stk| self.parse_define_event(stk))
                .await
                .map(DefineStatement::Event),
            t!("FIELD") => stk
                .run(|stk| self.parse_define_field(stk))
                .await
                .map(DefineStatement::Field),
            t!("INDEX") => stk
                .run(|stk| self.parse_define_index(stk))
                .await
                .map(DefineStatement::Index),
            t!("ANALYZER") => self
                .parse_define_analyzer(stk)
                .await
                .map(DefineStatement::Analyzer),
            t!("ACCESS") => self
                .parse_define_access(stk)
                .await
                .map(DefineStatement::Access),
            t!("CONFIG") => self
                .parse_define_config(stk)
                .await
                .map(DefineStatement::Config),
            t!("BUCKET") => self
                .parse_define_bucket(stk, next)
                .await
                .map(DefineStatement::Bucket),
            t!("SEQUENCE") => self
                .parse_define_sequence(stk)
                .await
                .map(DefineStatement::Sequence),
            t!("MODULE") => self
                .parse_define_module(stk)
                .await
                .map(DefineStatement::Module),
        ...
        loop {
            match self.peek_kind() {
                ...
                t!("THEN") => {
                    ...
                    while self.eat(t!(",")) {
                        res.then
                            .push(stk.run(|ctx| self.parse_expr_field(ctx)).await?)
                    }
                ...
                t!("RETRY") => {
                    let token = self.pop_peek();
                    if let EventKind::Async { retry, .. } = &mut res.event_kind {
                ...
                t!("MAXDEPTH") => {
                    let token = self.pop_peek();
                    if let EventKind::Async { max_depth, .. } = &mut res.event_kind {
    ...
    pub(crate) async fn parse_define_config(
        ...
        let inner = match next.kind {
            ...
            t!("DEFAULT") => self
                ...
                .await
                .map(ConfigInner::Default)?,
            _ => unexpected!(self, next, "a type of config"),
        };

        Ok(DefineConfigStatement { inner, kind })
    }
            ...
            match self.peek_kind() {
                ...
                t!("MIDDLEWARE") => {
                    ...
                    loop {
                        ...
                        middleware.push(Middleware { name, args });
    ...
    async fn parse_jwt(&mut self, stk: &mut Stk) -> ParseResult<access_type::JwtAccess> {
        ...
        match peek.kind {
            ...
            t!("URL") => {
                ...
                res.verify =
                    access_type::JwtAccessVerify::Jwks(access_type::JwtAccessVerifyJwks { url });
            }
