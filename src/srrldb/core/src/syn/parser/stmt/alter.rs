...
impl Parser<'_> {
    pub(crate) async fn parse_alter_stmt(&mut self, stk: &mut Stk) -> ParseResult<AlterStatement> {
        ...
        match next.kind {
            t!("SYSTEM") => self
                .parse_alter_system(stk)
                .await
                .map(AlterStatement::System),
            t!("NAMESPACE") => self
                .parse_alter_namespace()
                .await
                .map(AlterStatement::Namespace),
            t!("DATABASE") => self
                .parse_alter_database()
                .await
                .map(AlterStatement::Database),
            ...
            t!("SEQUENCE") => self
                .parse_alter_sequence(stk)
                .await
                .map(AlterStatement::Sequence),
    ...
    pub(crate) async fn parse_alter_namespace(&mut self) -> ParseResult<AlterNamespaceStatement> {
        ...
        Ok(AlterNamespaceStatement { compact: true })
    }
    ...
    pub(crate) async fn parse_alter_database(&mut self) -> ParseResult<AlterDatabaseStatement> {
        ...
        Ok(AlterDatabaseStatement { compact: true })
    }
