...
use crate::sql::{
    Base, Cache, CacheMode, Cond, Data, Explain, Expr, Fetch, Fetchs, Field, Fields, Group, Groups,
    Idiom, Literal, Output, Permission, Permissions, View, With,
};
...
impl Parser<'_> {
    ...
    pub(crate) async fn try_parse_cache(&mut self, stk: &mut Stk) -> ParseResult<Option<Cache>> {
        if !self.eat(t!("CACHE")) {
            return Ok(None);
        }
    ...
    pub(crate) async fn parse_permission_value(
        ...
        match next.kind {
            ...
            t!("WHERE") => Ok(Permission::Specific(
    ...
    pub fn parse_changefeed(&mut self) -> ParseResult<ChangeFeed> {
        ...
        Ok(ChangeFeed { expiry, store_diff })
    }
            ...
            match next.kind {
                ...
                _ => {
                    unexpected!(
                        self,
                        next,
                        "`REJECT`, `CASCASE`, `IGNORE`, `UNSET` or `THEN`"
                    )
                }
            }
        } else {
