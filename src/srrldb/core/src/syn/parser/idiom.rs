...
mod tests {
    ...
    fn graph_conditions_aliases() {
        ...
        let out = syn::expr(sql).unwrap();
        assert_eq!(
            "->(likes, follows WHERE influencer = true AS connections)",
            out.to_sql()
        );
    }
    ...
    fn graph_select() {
        ...
        let out = syn::expr(sql).unwrap();
        assert_eq!(
            "->(SELECT amount FROM likes WHERE amount > 10)",
            out.to_sql()
        );
    }
    ...
    fn graph_select_order_limit() {
        ...
        let out = syn::expr(sql).unwrap();
        assert_eq!(
            "->(SELECT amount FROM likes ORDER BY amount LIMIT 1)",
            out.to_sql()
        );
    }
    ...
    fn idiom_nested_array_all() {
        ...
        assert_eq!(
            out,
            sql::Expr::Idiom(Idiom(vec![f("test"), f("temp"), Part::All]))
        );
    }
    ...
    fn idiom_nested_array_last() {
        ...
        assert_eq!(
            out,
            sql::Expr::Idiom(Idiom(vec![f("test"), f("temp"), Part::Last]))
        );
    }
    ...
    fn idiom_nested_array_value() {
        ...
        assert_eq!(
            out,
            sql::Expr::Idiom(Idiom(vec![f("test"), f("temp"), Part::All, f("text")]))
        );
    }
