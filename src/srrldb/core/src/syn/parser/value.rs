...
impl Parser<'_> {
                ...
                | TokenKind::VectorType(_) = self.peek().kind
                    && let Some(x) = self
                        .speculate(stk, async |stk, this| {
                            ...
                            if this.eat(t!(",")) {
                                this.parse_value_object::<SurrealQL>(stk, open, res)
                                    .await
                                    .map(Some)
                            } else {
    ...
    pub async fn parse_json(&mut self, stk: &mut Stk) -> ParseResult<PublicValue> {
        ...
        match token.kind {
            ...
            t!("[") => {
                self.pop_peek();
                self.parse_value_array::<Json>(stk, token.span)
                    .await
                    .map(PublicValue::Array)
            }
            t!("\"") | t!("'") => {
            ...
            t!("-") | t!("+") | TokenKind::Digits => {
                ...
                match compound.value {
                    ...
                    Numeric::Integer(x) => Ok(PublicValue::Number(PublicNumber::Int(
                        x.into_int(compound.span)?,
                    ))),
            ...
            _ => {
                match self
                    .parse_value_record_id_inner::<Json>(stk)
                    .await
                    .map(PublicValue::RecordId)
                {
                    Ok(x) => Ok(x),
    ...
    async fn reparse_json_legacy_strand(&mut self, stk: &mut Stk, strand: String) -> PublicValue {
        if let Ok(x) = Parser::new(strand.as_bytes())
            .parse_value_record_id(stk)
            .await
        {
        ...
        let key = match peek.kind {
            ...
            t!("{") => {
                ...
                PublicRecordIdKey::Object(
                    self.parse_value_object::<VP>(stk, peek.span, BTreeMap::new())
                        .await?,
                )
            }
            t!("[") => {
