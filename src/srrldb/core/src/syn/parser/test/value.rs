...
fn parse_range_operator() {
    syn::parse_with("1..2".as_bytes(), async |parser, stk| {
        parser.parse_expr_field(stk).await
    })
    .unwrap();
}
...
fn parse_large_depth_object() {
    ...
    let query = stack
        .enter(|stk| parser.parse_expr_inherit(stk))
        .finish()
        .unwrap();
...
fn parse_i64() {
    let res = syn::parse_with(
        r#" -9223372036854775808 "#.as_bytes(),
        async |parser, stk| parser.parse_expr_field(stk).await,
    )
    .unwrap();
    ...
    let res = syn::parse_with(
        r#" 9223372036854775807 "#.as_bytes(),
        async |parser, stk| parser.parse_expr_field(stk).await,
    )
    .unwrap();
...
fn datetime_error() {
    syn::parse_with(
        r#" d"2001-01-01T01:01:01.9999999999" "#.as_bytes(),
        async |parser, stk| parser.parse_expr_field(stk).await,
    )
    .unwrap_err();
}
...
#[test]
fn empty_string() {
    syn::parse_with("".as_bytes(), async |parser, stk| {
        parser.parse_expr_field(stk).await
    })
    .unwrap_err();
}
