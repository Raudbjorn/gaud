...
fn less_then_idiom() {
	...
	"#;
    syn::parse_with(src.as_bytes(), async |parser, stk| {
...
#[test]
fn ident_is_field() {
    let mut field = syn::parse_with(r#"foo"#.as_bytes(), async |parser, stk| {
        parser.parse_query(stk).await
    })
    .unwrap();
...
fn test_non_valid_utf8() {
    ...
    stack
        .enter(|ctx| parser.parse_query(ctx))
        .finish()
        .unwrap_err();
}
