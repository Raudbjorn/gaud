...
#[test]
pub fn parse_begin() {
    let res = syn::parse_with(r#"BEGIN;"#.as_bytes(), async |parser, stk| {
...
fn parse_define_namespace() {
    let res = syn::parse_with(
        "DEFINE NAMESPACE a COMMENT 'test'".as_bytes(),
        async |parser, stk| parser.parse_expr_inherit(stk).await,
    )
    .unwrap();
    assert_eq!(
        res,
        Expr::Define(Box::new(DefineStatement::Namespace(
...
fn parse_define_database() {
    ...
    assert_eq!(
        res,
        Expr::Define(Box::new(DefineStatement::Database(
...
fn parse_define_function() {
    ...
    assert_eq!(
        res,
        Expr::Define(Box::new(DefineStatement::Function(
...
fn parse_define_user() {
    // Password.
    {
        ...
        let Expr::Define(res) = res else { panic!() };
    ...
    assert_eq!(
        ...
        Expr::Define(Box::new(DefineStatement::Table(DefineTableStatement {
            ...
            view: Some(crate::sql::View {
                ...
                cond: None,
                group: Some(Groups(vec![Group(Idiom(vec![Part::Field(
                    "foo".to_string()
                )]))]))
            }),
...
fn parse_define_analyzer() {
    ...
    assert_eq!(
        res,
        Expr::Define(Box::new(DefineStatement::Analyzer(
...
fn parse_delete() {
    ...
    assert_eq!(
        ...
        Expr::Delete(Box::new(DeleteStatement {
            only: true,
            what: vec![Expr::Mock(Mock::Range(
                "foo".to_string(),
                TypedRange::from_range(32..64)
            ))],
            with: Some(With::Index(vec![
                "index".to_string(),
                "index_2".to_string()
            ])),
    ...
    assert_eq!(
        ...
        Expr::IfElse(Box::new(IfelseStatement {
            exprs: vec![
                (
                    ident_field("foo"),
                    Expr::Block(Box::new(Block(vec![ident_field("bar")]))),
                ),
                (
                    ident_field("faz"),
                    Expr::Block(Box::new(Block(vec![ident_field("baz")]))),
                )
            ],
...
fn parse_info() {
    ...
    let res = syn::parse_with(
        "INFO FOR USER user ON namespace".as_bytes(),
        async |parser, stk| parser.parse_expr_inherit(stk).await,
    )
    .unwrap();
    assert_eq!(
...
fn parse_use() {
    ...
    .pop()
    .unwrap();
    let expect = TopLevelExpr::Use(UseStatement::Ns(Expr::Idiom(Idiom::field(
        "foo".to_owned(),
    ))));
    assert_eq!(res, expect);
...
fn parse_use_lowercase() {
    ...
    let expect = TopLevelExpr::Use(UseStatement::Db(Expr::Idiom(Idiom::field(
        "foo".to_owned(),
    ))));
    assert_eq!(res, expect);
...
#[test]
fn parse_value_stmt() {
    let res = syn::parse_with(r"1s".as_bytes(), async |parser, stk| {
        parser.parse_expr_inherit(stk).await
    })
    .unwrap();
...
fn parse_throw() {
    ...
    let expect = Expr::Throw(Box::new(Expr::Literal(Literal::Duration(
    ...
    assert_eq!(
        ...
        Expr::Insert(Box::new(InsertStatement {
            ...
            data: Data::ValuesExpression(vec![
                vec![
                    (
                        Idiom::field("a".to_owned()),
                        Expr::Literal(Literal::Integer(1)),
                    ),
                    (
                        Idiom::field("b".to_owned()),
                        Expr::Literal(Literal::Integer(2)),
                    ),
                    (
                        Idiom::field("c".to_owned()),
...
fn parse_live() {
    let res = syn::parse_with(
        r#"LIVE SELECT DIFF FROM $foo"#.as_bytes(),
        async |parser, stk| parser.parse_top_level_expr(stk).await,
    )
    .unwrap();
...
fn parse_return() {
    let res = syn::parse_with(
        r#"RETURN RETRUN FETCH RETURN"#.as_bytes(),
        async |parser, stk| parser.parse_expr_inherit(stk).await,
    )
    .unwrap();
    assert_eq!(
...
fn parse_remove() {
    ...
    assert_eq!(
        res,
        Expr::Remove(Box::new(RemoveStatement::Namespace(
    ...
    assert_eq!(
        res,
        Expr::Remove(Box::new(RemoveStatement::Database(
    ...
    let res = syn::parse_with(
        r#"REMOVE FUNCTION fn::foo::bar"#.as_bytes(),
        async |parser, stk| parser.parse_expr_inherit(stk).await,
    )
    .unwrap();
    assert_eq!(
        res,
        Expr::Remove(Box::new(RemoveStatement::Function(
    ...
    let res = syn::parse_with(
        r#"REMOVE ACCESS foo ON DATABASE"#.as_bytes(),
        async |parser, stk| parser.parse_expr_inherit(stk).await,
    )
    .unwrap();
    assert_eq!(
    ...
    let res = syn::parse_with(
        r#"REMOVE EVENT foo ON TABLE bar"#.as_bytes(),
        async |parser, stk| parser.parse_expr_inherit(stk).await,
    )
    .unwrap();
    assert_eq!(
    ...
    let res = syn::parse_with(
        r#"REMOVE FIELD foo.bar[10] ON bar"#.as_bytes(),
        async |parser, stk| parser.parse_expr_inherit(stk).await,
    )
    .unwrap();
    assert_eq!(
    ...
    let res = syn::parse_with(
        r#"REMOVE INDEX foo ON bar"#.as_bytes(),
        async |parser, stk| parser.parse_expr_inherit(stk).await,
    )
    .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        res,
        Expr::Remove(Box::new(RemoveStatement::Analyzer(
    ...
    let res = syn::parse_with(
        r#"REMOVE user foo on database"#.as_bytes(),
        async |parser, stk| parser.parse_expr_inherit(stk).await,
    )
    .unwrap();
    assert_eq!(
