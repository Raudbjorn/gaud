...
pub fn init(dbs: Arc<Datastore>, canceller: CancellationToken, opts: &EngineOptions) -> Tasks {
	...
	let task6 = spawn_task_event_processing(dbs, canceller.clone(), opts);
	let task7 = spawn_task_query_cache_cleanup(canceller, opts);
	Tasks(vec![task1, task2, task3, task4, task5, task6, task7])
}
