...
//! ```toml
//! [dependencies]
//! antigravity-gate = { version = "0.1", features = ["keyring"] }
//! ```
...
const SERVICE_NAME: &str = "antigravity-gate";
...
///
/// ```rust,ignore
/// use antigravity_gate::storage::KeyringTokenStorage;
///
...
impl KeyringTokenStorage {
    ...
    ///
    /// Uses service name "antigravity-gate" and account "oauth-token".
    pub fn new() -> Self {
        Self {
    ...
    pub fn is_available() -> bool {
        ...
        match Entry::new("antigravity-gate-test", "availability-check") {
            Ok(entry) => {
...
mod tests {
    ...
    fn test_new() {
        let storage = KeyringTokenStorage::new();
        assert_eq!(storage.service(), "antigravity-gate");
