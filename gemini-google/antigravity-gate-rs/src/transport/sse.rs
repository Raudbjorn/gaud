...
use crate::convert::GLOBAL_SIGNATURE_CACHE;
use crate::error::{Error, Result};
use crate::models::content::ContentBlock;
use crate::models::response::{StopReason, Usage};
use crate::models::stream::{ContentDelta, MessageDelta, PartialMessage, StreamError, StreamEvent};

pin_project! {
    /// SSE stream parser that converts Cloud Code responses to Anthropic events.
    ///
    /// This struct wraps a byte stream and parses SSE events, converting them
    /// from Google Generative AI format to Anthropic `StreamEvent` format.
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// use antigravity_gate::transport::SseStream;
    ///
    /// let response = http_client.post_stream(&url, &headers, &body).await?;
    /// let mut stream = SseStream::new(response.bytes_stream(), "claude-sonnet-4-5");
    ///
    ...
    S: Stream<Item = std::result::Result<Bytes, reqwest::Error>> + Send + 'static,
{
    ...
    /// * `model` - The model name (used for signature caching)
...
/// Parse an SSE byte stream into Anthropic stream events.
