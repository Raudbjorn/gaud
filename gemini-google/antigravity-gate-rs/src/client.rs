...
//!
//! ```rust,ignore
//! use antigravity_gate::{CloudCodeClient, FileTokenStorage, MessagesRequest};
//!
...
use crate::transport::http::{generate_request_id, HttpClient};
...
impl<S: TokenStorage + 'static> CloudCodeClient<S> {
    ...
    /// ```
    pub async fn start_oauth_flow(&self) -> Result<(String, crate::auth::OAuthFlowState)> {
    ...
    pub async fn request(&self, request: &MessagesRequest) -> Result<MessagesResponse> {
        ...
        // Parse response
        let google_response: crate::models::google::GoogleResponse = response.json().await?;
        ...
        while let Some(event) = sse_stream.next().await {
            ...
            match event {
                ...
                StreamEvent::ContentBlockDelta { delta, .. } => {
                    match delta {
                        crate::models::stream::ContentDelta::TextDelta { text } => {
                            current_text.push_str(&text);
                        }
                        crate::models::stream::ContentDelta::ThinkingDelta { thinking } => {
                            current_thinking.push_str(&thinking);
                        }
                        crate::models::stream::ContentDelta::SignatureDelta { signature } => {
                            current_signature = Some(signature);
                        }
                        crate::models::stream::ContentDelta::InputJsonDelta { partial_json: _ } => {
    ...
    fn wrap_request(
        ...
        let _session_id = derive_session_id(original_request);
        ...
        // Add session_id at request level for caching
        // Note: This requires adding session_id to GoogleRequest if not already present
        // For now, the session_id is part of the wrapper metadata
...
pub struct CloudCodeClientBuilder<S: TokenStorage> {
    storage: Option<S>,
    http_builder: crate::transport::http::HttpClientBuilder,
    oauth_config: Option<crate::constants::OAuthConfig>,
...
impl<S: TokenStorage + 'static> CloudCodeClientBuilder<S> {
    ...
    pub fn with_oauth_config(mut self, config: crate::constants::OAuthConfig) -> Self {
...
impl<S: TokenStorage> Default for CloudCodeClientBuilder<S> {
    fn default() -> Self {
        Self {
            storage: None,
            http_builder: crate::transport::http::HttpClientBuilder::default(),
            oauth_config: None,
...
impl<S: TokenStorage + 'static> MessagesRequestBuilder<S> {
    ...
    pub fn system_blocks(mut self, blocks: Vec<crate::models::request::SystemBlock>) -> Self {
    ...
    pub async fn send_stream(self) -> Result<impl futures::Stream<Item = Result<StreamEvent>>> {
        let request = self.build_request()?;
