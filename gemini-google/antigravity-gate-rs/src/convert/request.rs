...
///
/// ```rust,ignore
/// use antigravity_gate::convert::convert_request;
/// use antigravity_gate::MessagesRequest;
///
        ...
        if !tools.is_empty() {
            ...
            if is_claude {
                google_request.tool_config = Some(ToolConfig {
                    function_calling_config: crate::models::google::FunctionCallingConfig {
...
fn convert_tools(tools: &[crate::models::tools::Tool]) -> Vec<FunctionDeclaration> {
    tools
        .iter()
        .enumerate()
        .map(|(idx, tool)| {
...
fn convert_tool_choice(choice: &crate::models::tools::ToolChoice) -> ToolConfig {
    match choice {
        crate::models::tools::ToolChoice::Auto => ToolConfig::auto(),
        crate::models::tools::ToolChoice::Any => ToolConfig::any(),
        crate::models::tools::ToolChoice::None => ToolConfig::none(),
        crate::models::tools::ToolChoice::Tool { name } => ToolConfig::force(name),
    }
}
...
mod tests {
    ...
    fn test_convert_tool_choice_auto() {
        let config = convert_tool_choice(&crate::models::tools::ToolChoice::Auto);
        assert_eq!(config.function_calling_config.mode, "AUTO");
    }

    #[test]
    fn test_convert_tool_choice_any() {
        let config = convert_tool_choice(&crate::models::tools::ToolChoice::Any);
        assert_eq!(config.function_calling_config.mode, "ANY");
    }

    #[test]
    fn test_convert_tool_choice_none() {
        let config = convert_tool_choice(&crate::models::tools::ToolChoice::None);
        assert_eq!(config.function_calling_config.mode, "NONE");
    }

    #[test]
    fn test_convert_tool_choice_specific() {
        let config = convert_tool_choice(&crate::models::tools::ToolChoice::tool("my_tool"));
        assert_eq!(config.function_calling_config.mode, "ANY");
        assert_eq!(
    ...
    fn test_convert_system_blocks() {
        use crate::models::request::SystemBlock;
